...
app.controller('ManageLeaveMasterController',
    function($scope, $http, $filter) {
        ...
        $scope.GetEmpInfo = function() {
            ...
            emp.onreadystatechange = function() {
                if (
                    ...
                    $scope.empdata = json1;
                    $scope.$digest();
                    $scope.GetLARelesaseStratey();
                    $scope.GetGPRelesaseStratey();
                    $scope.GetLeaveRequestLists();
                    $scope.GetLeave();
                    $scope.GetPostedLeave();
        ...
        $scope.GetLeaveRequestLists = function() {
            ...
            xhr1.open('GET',
                $scope._Conpath + 'LeaveApplication/GetLeaveApplication?empUnqId=' + $('#eCode').val(),
                true);
                ...
                if (
                    ...
                    for (var i = 0; i < la.length; i++) {
                        ...
                        for (var l = 0;
                            ...
                            myArray[cnt]["applReleaseStatus"] = la[i].applReleaseStatus;
                            ...
                            myArray[cnt]["postedBy"] = la[i].updUser;
                            ...
                            myArray[cnt]["coDate1"] = leaveApplicationDetails[l].coDate1;
                            myArray[cnt]["coDate2"] = leaveApplicationDetails[l].coDate2;
                            myArray[cnt]["isPosted"] = leaveApplicationDetails[l].isPosted;
                            myArray[cnt]["cancelled"] = leaveApplicationDetails[l].cancelled;
                            myArray[cnt]["parentId"] = leaveApplicationDetails[l].parentId;
                            myArray[cnt]["postUser"] = leaveApplicationDetails[l].postUser;
                            myArray[cnt]["postedDt"] = leaveApplicationDetails[l].postedDt;
                            cnt++;
                        }
                        leaveApplicationDetails = "";
                    }
                    $scope.lappdata = myArray;
        ...
        $scope.PopulateData = function(leaveappid, leaveappitem) {
            ...
            $scope.leaveappdetails = "";
            $scope.GetLeaveApplicationDetails(leaveappid, leaveappitem);
        };
        $scope.GetLeaveApplicationDetails = function(leaveappid, leaveappitem) {
            var xhr1 = new XMLHttpRequest();
            xhr1.open('GET', $scope._Conpath + 'LeaveApplication/GetLeaveApplication?leaveAppId=' + leaveappid, true);
            xhr1.setRequestHeader('Accept', 'application/json');
            xhr1.onreadystatechange = function() {
                if (xhr1.readyState === 4) {
                    var jsonvar = JSON.parse(xhr1.responseText);
                    var leavedetail = jsonvar[0].leaveApplicationDetails;
                    for (var i = 0; i < leavedetail.length; i++) {
                        var leaveitemno = leavedetail[i].leaveAppItem;
                        if (leaveitemno === leaveappitem) {
                            $scope.leaveappdetails = jsonvar[0].leaveApplicationDetails[i];
                            $scope.$digest();
                            $('#FromDt').val(leavedetail[i].fromDt.replace("T00:00:00", ""));
                            $('#ToDt').val(leavedetail[i].toDt.replace("T00:00:00", ""));
                            $('#Remarks').val(leavedetail[i].remarks);
                            var hlf = leavedetail[i].halfDayFlag;
                            if (hlf === true) {
                                $("#HalfFlag").attr("checked", true);
                            } else {
                                $("#HalfFlag").attr("checked", false);
                            };
                            $('#TotalDays').val(leavedetail[i].totalDays);
                            //Get Select object & Set Leave Type in the Selection
                            var objSelect = document.getElementById("LeaveType");
                            setSelectedValue(objSelect, leavedetail[i].leaveTypeCode);
        ...
        $scope.createLeave = function() {
            ...
            var LeaveTypeCode = strUser;
            if (LeaveTypeCode.length > 2) {
                LeaveTypeCode = $scope.leaveappdetails.leaveTypeCode;
            };
            ...
            if (HalfDayFlag === true) {
                ...
                if (Totaldays === null || Totaldays === '' || Totaldays === 0) {
                    Totaldays = diff;
                };
            };
            var jsonObj = {};
            ...
            function storeTblValues() {
                jsonObj.yearMonth = $scope.leaveappdetails.yearMonth;
                jsonObj.leaveAppId = $scope.leaveappdetails.leaveAppId;
                jsonObj.compCode = $scope.leaveappdetails.compCode;
                jsonObj.wrkGrp = $scope.leaveappdetails.wrkGrp;
                jsonObj.leaveAppItem = $scope.leaveappdetails.leaveAppItem;
                jsonObj.leaveTypeCode = LeaveTypeCode;
                jsonObj.fromDt = FromDate;
                jsonObj.toDt = ToDate;
                jsonObj.totalDays = Totaldays;
                jsonObj.halfdayflag = HalfDayFlag;
                jsonObj.remarks = Remarks;
                return jsonObj;
            };

            var la = new XMLHttpRequest();
            la.open('POST', $scope._Conpath + 'ManageLeave/ManageLeave?EmpUnqID=' + $('#eCode').val(), true);
            ...
            la.onreadystatechange = function() {
                if (la.readyState === 4 && la.status === 200) {
                    ...
                    document.getElementById("MessageBox").innerHTML =
                        "<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Leave Application Updated.. </strong></div>";
                    $('#MessageBox').show();
                    ...
                    jQuery('#btnClose').click();
                    document.getElementById("MessageBox").innerHTML =
                        "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Leave Application Not Updated.. </strong></div>";
                    $('#MessageBox').show();
                    document.getElementById("FromDt").value = "";
                    document.getElementById("ToDt").value = "";
                    document.getElementById("TotalDays").value = "";
        ...
        $scope.GetLARelesaseStratey = function() {
            ...
            LA.open('GET',
                ...
                $('#eCode').val(),
                true);
            LA.setRequestHeader('Accept', 'application/json');
            LA.onreadystatechange = function() {
                if (LA.readyState === 4) {
                    var jsonvar1 = JSON.parse(LA.responseText);
        ...
        $scope.LeaveReject = function(rejRemarks, value, value1) {
            ...
            function storeValues() {
                ...
                $('#aliasTable tr').each(function(row, tr) {
                    tbldata[row] = {
                        "LeaveAppId": $(tr).find('td:eq(5)').text(),
                        "LeaveAppItem": $(tr).find('td:eq(7)').text()
                    }
                });
                var tbl = new Array();
                tbl[0] = "test";
                ...
                for (var i = 0; i < tbldata.length; i++) {
                    ...
                    if (appid == value) {
                        if ((typeof (rejRemarks) === "undefined") || (typeof (rejRemarks.remarks) === "undefined")) {
                            document.getElementById("MessageBox").innerHTML =
                                "<div class='alert alert-warning alert-dismissable'>" +
                                "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                                "<strong>Please Enter Remarks First</strong></div>";
                            $('#MessageBox').show();
                            return;
                        };
                        tbl[count] = {
                            "YearMonth": value1,
                            "LeaveAppId": value,
                            "LeaveAppItem": tbldata[i]["LeaveAppItem"],
                            "IsPosted": "R",
                            "Remarks": rejRemarks.remarks,
            ...
            var rej = new XMLHttpRequest();
            rej.open('POST', $scope._Conpath + 'LeavePosting/PostLeaves', true);
            rej.setRequestHeader("Content-type", "application/json");
        ...
        $scope.GetLeave = function() {
            ...
            req.open('GET',
                $scope._Conpath +
                'LeaveBalance/GetLeaveBalance?empUnqId=' +
                $('#eCode').val() +
                '&yearmonth=' +
                d.getFullYear(),
                true);
        ...
        $scope.GetPostedLeave = function() {
            ...
            pstl.open('GET', $scope._Conpath + 'leaveentry?empunqid=' + $('#eCode').val(), true);
        ...
        return {
            ...
            link: function(scope, elem, attrs, ngModelCtrl) {
                var updateModel = function(dateText) {
                    scope.$apply(function() { ngModelCtrl.$setViewValue(dateText); });
                };
...
app.controller('SelectLTypeCntrl',
    function($scope, $http) {
        $scope.GetLTypeList = function() {
            $http({
                    method: 'Get',
                    contentType: "application/json",
                    url: '/LeaveApp/GetLeaveTypeList?compid=' + $('#myCompCode').val()
                }).success(function(data, status, headers, config) { $scope.LList = data; })
                .error(function(data, status, headers, config) { $scope.message = 'Unexpected Error'; });
        };
    });