...
app.controller('ManageLeaveMasterController', function ($scope, $http, $filter) {
    ...
    $scope.GetLeaveRequestLists = function () {
        var xhr1 = new XMLHttpRequest(); xhr1.open('GET', $scope._Conpath + 'LeaveApplication/GetLeaveApplication?empUnqId=' + $('#eCode').val(), true); xhr1.setRequestHeader('Accept', 'application/json');
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4) {
                ...
                $scope.curPage = 0; $scope.pageSize = 10;
                $scope.$digest();
            }
        };
        xhr1.send();
                ...
                for (var i = 0; i < leavedetail.length; i++) {
                    ...
                    if (leaveitemno === leaveappitem) {
                        ...
                        function setSelectedValue(selectObj, valueToSet) {
                            for (var i = 0; i < selectObj.options.length; i++) { if (selectObj.options[i].text === valueToSet) { selectObj.options[i].selected = true; return; } }
                        }
                    };
                }
            }
    ...
    $scope.createLeave = function () {
        ...
        var la = new XMLHttpRequest(); la.open('POST', $scope._Conpath + 'ManageLeave/ManageLeave?EmpUnqID=' + $('#eCode').val(), true); la.setRequestHeader("Content-type", "application/json");
