...
app.controller('ManageLeaveMasterController', function ($scope, $http, $filter) {
    ...
    $scope.currentPage = 1; $scope.itemsPerPage = 10; $scope.alluserlist = []; $scope.leaveappdetails = "";
    ...
    $scope.ToValidate = function () { var chkFrom = document.getElementById('FromDt'); var chkTo = document.getElementById('ToDt'); var FromDate = chkFrom.value; var ToDate = chkTo.value; var date1 = new Date(FromDate); var date2 = new Date(ToDate); var diff = ((date1 - date2) / (1000 * 60 * 60 * 24) * -1) + 1; $('#TotalDays').text = diff; document.getElementById("TotalDays").value = diff; if (diff > 1) { $("#HalfFlag").attr("disabled", true); $("#HalfFlag").attr("checked", false); } else { $("#HalfFlag").removeAttr("disabled"); } if (date2 < date1) { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Please Enter Valid Date Range.. </strong></div>"; $('#MessageBox').show(); return false; } else { return true; } };
    //Get Employee details from employee code (Method Use for Reset Password)
    $scope.GetEmpInfo = function () { var emp = new XMLHttpRequest(); emp.open('GET', $scope._Conpath + 'Employee/GetEmployee?empunqid=' + $('#eCode').val(), true); emp.setRequestHeader('Accept', 'application/json'); emp.onreadystatechange = function () { if (emp.readyState === 4) { var json1 = JSON.parse(emp.responseText); $scope.empdata = json1; $scope.$digest(); $scope.GetRelesaseStratey(); $scope.GetLeaveRequestLists(); $scope.GetLeave(); $scope.GetPostedLeave(); } else if (emp.status !== 200) { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Record Not Found.. </strong></div>"; $('#MessageBox').show(); } }; emp.send(); };
    $scope.GetLeaveRequestLists = function () { var xhr1 = new XMLHttpRequest(); xhr1.open('GET', $scope._Conpath + 'LeaveApplication/GetLeaveApplication?empUnqId=' + $('#eCode').val(), true); xhr1.setRequestHeader('Accept', 'application/json'); xhr1.onreadystatechange = function () { if (xhr1.readyState === 4) { var json = JSON.parse(xhr1.responseText); var la = new Array; la = json; var applReleaseStatus = new Array; var leaveApplicationDetails = new Array; var empName, statName, releaseDate; var cnt = 0; var myArray = []; for (var i = 0; i < la.length; i++) { empName = la[i].employee.empName; statName = la[i].stations.statName; applReleaseStatus = la[i].applReleaseStatus; for (var j = 0; j < applReleaseStatus.length; j++) { releaseDate = applReleaseStatus[j].releaseDate; }; leaveApplicationDetails = la[i].leaveApplicationDetails; for (var l = 0; l < leaveApplicationDetails.length; l++) { myArray.push([]); myArray[cnt]["releaseGroupCode"] = la[i].releaseGroupCode; myArray[cnt]["releaseStatusCode"] = la[i].releaseStatusCode; myArray[cnt]["leaveAppId"] = la[i].leaveAppId; myArray[cnt]["yearMonth"] = la[i].yearMonth; myArray[cnt]["addDt"] = la[i].addDt; myArray[cnt]["releaseDate"] = releaseDate; myArray[cnt]["updDt"] = la[i].updDt; myArray[cnt]["empUnqId"] = la[i].empUnqId; myArray[cnt]["empName"] = empName; myArray[cnt]["statName"] = statName; myArray[cnt]["postedBy"] = la[i].updUser; myArray[cnt]["relremarks"] = la[i].remarks; myArray[cnt]["remarks"] = leaveApplicationDetails[l].remarks; myArray[cnt]["leaveAppItem"] = leaveApplicationDetails[l].leaveAppItem; myArray[cnt]["leaveTypeCode"] = leaveApplicationDetails[l].leaveTypeCode; myArray[cnt]["fromDt"] = leaveApplicationDetails[l].fromDt; myArray[cnt]["toDt"] = leaveApplicationDetails[l].toDt; myArray[cnt]["totalDays"] = leaveApplicationDetails[l].totalDays; myArray[cnt]["halfDayFlag"] = leaveApplicationDetails[l].halfDayFlag; myArray[cnt]["isPosted"] = leaveApplicationDetails[l].isPosted; myArray[cnt]["cancelled"] = leaveApplicationDetails[l].cancelled; myArray[cnt]["parentId"] = leaveApplicationDetails[l].parentId; myArray[cnt]["postUser"] = leaveApplicationDetails[l].postUser; myArray[cnt]["postedDt"] = leaveApplicationDetails[l].postedDt; cnt++; } leaveApplicationDetails = ""; } $scope.lappdata = myArray; $scope.lappdata = $filter('orderBy')($scope.lappdata, '-leaveAppId'); $scope.curPage = 0; $scope.pageSize = 10; $scope.$digest(); } }; xhr1.send(); };
    ...
    $scope.GetLeaveApplicationDetails = function (leaveappid, leaveappitem) {
        var xhr1 = new XMLHttpRequest(); xhr1.open('GET', $scope._Conpath + 'LeaveApplication/GetLeaveApplication?leaveAppId=' + leaveappid, true); xhr1.setRequestHeader('Accept', 'application/json');
            ...
            if (xhr1.readyState === 4) {
                ...
                for (var i = 0; i < leavedetail.length; i++) {
                    ...
                    if (leaveitemno === leaveappitem) {
                        $scope.leaveappdetails = jsonvar[0].leaveApplicationDetails[i]; $scope.$digest(); $('#FromDt').val(leavedetail[i].fromDt.replace("T00:00:00", "")); $('#ToDt').val(leavedetail[i].toDt.replace("T00:00:00", "")); $('#Remarks').val(leavedetail[i].remarks); var hlf = leavedetail[i].halfDayFlag; if (hlf === true) { $("#HalfFlag").attr("checked", true); } else { $("#HalfFlag").attr("checked", false); }; $('#TotalDays').val(leavedetail[i].totalDays);
...
//Get Leave Types
app.controller('SelectLTypeCntrl', function ($scope, $http) { $scope.GetLTypeList = function () { $http({ method: 'Get', contentType: "application/json", url: '/LeaveApp/GetLeaveTypeList?compid=' + $('#myCompCode').val() }).success(function (data, status, headers, config) { $scope.LList = data; }).error(function (data, status, headers, config) { $scope.message = 'Unexpected Error'; }); }; });