angular.module('myApp.Controllers').controller("ReleaseStreategyCntrloller", ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.GetEmpInfo = function () {
        $('#tbl_empdtl').show();
        var e_Code = $('#eCode').val(); if (e_Code === '') { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-info alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Please Enter Employee Code First.. </strong></div>"; $('#MessageBox').show(); return false; }
    ...
    $scope.CreateReleaseStrategy = function () {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                $("#aliasTable").find("tr:not(:first)").remove(); $("#tbl_rlsdtl").find("tr:not(:first)").remove(); document.getElementById("eCode").value = ""; document.getElementById("Rel_Empid").value = "";
            }
            else {
