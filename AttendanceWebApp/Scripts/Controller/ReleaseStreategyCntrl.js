angular.module('myApp.Controllers').controller("ReleaseStreategyCntrloller", ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.GetEmpInfo = function () {
        ...
        if (e_Code === '') { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-info alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Please Enter Employee Code First.. </strong></div>"; $('#MessageBox').show(); return false; }
    ...
    $scope.GetReleaserInfo = function () {
        ...
        if (rls_Code === '') { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-info alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Please Enter Releaser Employee Code.. </strong></div>"; $('#MessageBox').show(); return false; }
        ...
        rls.onreadystatechange = function () {
            if (rls.readyState === 4) { var json2 = JSON.parse(rls.responseText); $scope.rdata = json2; $scope.$digest(); }
            else if (rls.status !== 200) { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Record Not Found.. </strong></div>"; $('#MessageBox').show(); }
        };
        rls.send();
    };
    ...
    $scope.AddReleaser = function (data) {
        ...
        if ($scope.rdata.length > 0) {
            var rls = $scope.rdata; var rlscode = data; var rls_Code = $('#Rel_Empid').val();
            for (var i = 0; i <= rls.length; i++) {
                var r_code = rls[i]["releaseCode"]; var r_empid = rls[i]["empUnqId"];
    ...
    $scope.CreateReleaseStrategy = function () {
        var d2 = new Date(); var today = new Date(d2.getFullYear(), d2.getMonth() + 1, d2.getDate(), d2.getHours(), d2.getMinutes(), d2.getSeconds());
        var now = (today.getFullYear()) + '/' + (today.getMonth()) + '/' + today.getDate() + ' ' + today.getHours() + ':' + today.getMinutes() + ':' + today.getSeconds();
        var tables = document.getElementById('aliasTable'); var rowCounts = tables.rows.length;
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                ...
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Release Strategy Not Saved..</strong></div>"; $('#MessageBox').show();
                document.getElementById("eCode").value = "";
                document.getElementById("Rel_Empid").value = "";
            }
        };
