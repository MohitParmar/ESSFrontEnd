...
app.controller('LeaveReportCntrloller', function ($scope, $http, $filter) {
    ...
    $scope.GetLeaveInfo = function (entity) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                var json = JSON.parse(xhr.responseText); $scope.data = json;
    ...
    $scope.PerformanceAttendanceRpt = function (str) {
        ...
        pr.onreadystatechange = function () {
            if (pr.readyState === 4) {
                ...
                $scope.curPage = 0; $scope.pageSize = 31;
                $scope.$digest();
            }
        };
        pr.send();
    ...
    $scope.exportAllData = function (name) {
        setTimeout(function () {
            ...
            var FileName = name + d; $scope.JSONToCSVConvertor($scope.InfoPL, FileName, true);
            $('#loading').removeClass("activediv"); $('#loading').addClass("deactivediv");
        }, 100);
    };
    $scope.JSONToCSVConvertor = function (JSONData, ReportTitle, ShowLabel) {
        ...
        var fileName = "Report_"; fileName += ReportTitle.replace(/ /g, "_");
