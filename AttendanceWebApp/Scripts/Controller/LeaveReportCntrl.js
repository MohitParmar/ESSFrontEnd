...
app.controller('LeaveReportCntrloller', function ($scope, $http, $filter) {
    ...
    $scope.ToValidate = function () {
        var chkFrom = document.getElementById('FromDt'); var chkTo = document.getElementById('ToDt'); var FromDate = chkFrom.value; var ToDate = chkTo.value; var date1 = new Date(FromDate); var date2 = new Date(ToDate);
    ...
    $scope.GetPostedLeaveInfo = function (data) {
        ...
        var xhr1 = new XMLHttpRequest(); xhr1.open('GET', $scope._Conpath + 'LeavePosting/GetLeaves?fromDt=' + FromDate + '&toDt=' + ToDate + '&postingFlg=' + sel, true); xhr1.setRequestHeader('Accept', 'application/json');
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4) {
                ...
                var la = new Array; la = json; var applReleaseStatus = new Array; var leaveApplicationDetails = new Array; var empName, statName, releaseDate, location;
                var cnt = 0; var myArray = [];
                for (var i = 0; i < la.length; i++) {
                    ...
                    for (var l = 0; l < leaveApplicationDetails.length; l++) {
                        myArray.push([]);
                        myArray[cnt]["location"] = location; myArray[cnt]["leaveAppId"] = la[i].leaveAppId; myArray[cnt]["addDt"] = la[i].addDt.substring(0, la[i].addDt.indexOf("T"));
                        myArray[cnt]["releaseDate"] = releaseDate ? releaseDate.substring(0, releaseDate.indexOf("T")) : '';
                        myArray[cnt]["postedDt"] = leaveApplicationDetails[l].postedDt.substring(0, leaveApplicationDetails[l].postedDt.indexOf("T"));
                        myArray[cnt]["empUnqId"] = la[i].empUnqId; myArray[cnt]["empName"] = empName; myArray[cnt]["statName"] = statName;
                        myArray[cnt]["remarks"] = leaveApplicationDetails[l].remarks; myArray[cnt]["leaveAppItem"] = leaveApplicationDetails[l].leaveAppItem;
                        myArray[cnt]["leaveTypeCode"] = leaveApplicationDetails[l].leaveTypeCode; myArray[cnt]["isPosted"] = leaveApplicationDetails[l].isPosted;
                        ...
                        myArray[cnt]["totalDays"] = leaveApplicationDetails[l].totalDays; myArray[cnt]["halfDayFlag"] = leaveApplicationDetails[l].halfDayFlag;
                        cnt++;
    ...
    $scope.PerformanceAttendanceRpt = function (str) {
        ...
        if (str === "PERF") { $('#rptPerformance').show(); $('#rptPunching').hide(); } if (str === "PUNCH") { $('#rptPerformance').hide(); $('#rptPunching').show(); }
        var pr = new XMLHttpRequest(); pr.open('GET', $scope._Conpath + 'Employee/PerfAttd?empunqid=' + $('#myEmpUnqId').val() + '&flag=' + str + '&fromdate=' + FromDate + '&todate=' + ToDate, true); pr.setRequestHeader('Accept', 'application/json');
        pr.onreadystatechange = function () { if (pr.readyState === 4) { $scope.InfoPL = pr.responseText; var json = JSON.parse(pr.responseText); $scope.prdata = json; if (str === "PERF") { $scope.prdata = $filter('orderBy')($scope.prdata, '-attdDate'); } if (str === "PUNCH") { $scope.prdata = $filter('orderBy')($scope.prdata, '-punchDate'); } $scope.curPage = 0; $scope.pageSize = 31; $scope.$digest(); } }; pr.send();
    };
