angular.module("myApp.Controllers").controller("LockDownController", ["$scope", "$http", function ($scope, $http) {
    ...
    $scope.ToValidate = function () {
        ...
        if (FromDate === "") {
            ...
            $("#Remarks option:first").attr("selected", true);
            return false;
        }
        ...
        if (diff === "NaN" || diff <= 0) {
            document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" + "<strong>Please Select Proper Dates for Leave apply.</strong></div>";
            $("#MessageBox").show();
            document.getElementById("FromDt").value = "";
            document.getElementById("ToDt").value = "";
            document.getElementById("TotalDays").value = "";
    ...
    $scope.LeaveRequestData = function (entity) {
        ...
        var chkohsl = false;
        if (typeof entity === "undefined" || typeof entity.FromDt === "undefined" || typeof entity.Remarks === "undefined" || typeof entity.AdditionalRemarks === "undefined") {
            alert("Please Fill All Required Details Step by Step...");
            return false;
        }
        var LeaveTypeCode = "LD";
        var HalfDayFlag = false;
        var Totaldays = 0;
        var Remarks = $("#Remarks option:selected").text();
        var AdditionalRemarks = entity.AdditionalRemarks.replace("'", "");
        ...
        function storeTblValues() {
            ...
            $("#aliasTable tr").each(function (row, tr) {
                TableData[row] = {
                    ...
                    additionalRemarks: $(tr).find("td:eq(6)").text()
                };
            });
            TableData.shift();
        ...
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4 && xhr1.status === 200) {
                ...
                var row = $("<tr>" + "<td style='text-align:center;'><input type='hidden' name='AliasFirstNames' value='" + dtlarr[dtlarrleg]["leaveAppItem"] + "'>" + dtlarr[dtlarrleg]["leaveAppItem"] + "</td>" + "<td style='text-align:center;'><input type='hidden' name='AliasFirstNames' value='" + dtlarr[dtlarrleg]["leaveTypeCode"] + "'>" + dtlarr[dtlarrleg]["leaveTypeCode"] + "</td>" + "<td style='text-align:center;'><input type='hidden' name='AliasMiddleNames' value='" + fdate + "'>" + fdate + "</td>" + "<td style='text-align:center;'><input type='hidden' name='AliasLastNames' value='" + tdate + "'>" + tdate + "</td>" + "<td style='text-align:center;'><input type='hidden' name='AliasLastNames' value='" + dtlarr[dtlarrleg]["totalDays"] + "'>" + dtlarr[dtlarrleg]["totalDays"] + "</td>" + "<td style='text-align:center;'><input type='hidden' name='AliasLastNames' value='" + dtlarr[dtlarrleg]["remarks"] + "'>" + dtlarr[dtlarrleg]["remarks"] + "</td>" + "<td style='text-align:center;'><input type='hidden' name='AliasLastNames' value='" + dtlarr[dtlarrleg]["additionalRemarks"] + "'>" + dtlarr[dtlarrleg]["additionalRemarks"] + "</td>" + "</tr>");
                $("#aliasTable").append(row);
                document.getElementById("FromDt").value = "";
                document.getElementById("ToDt").value = "";
                document.getElementById("TotalDays").value = "";
    ...
    $scope.createLeave = function () {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 201) {
                ...
                rlsmail.open("GET", $scope._Conpath + "AutoMail/SendMail?releaseGroupCode=" + maildata["releaseGroupCode"] + "&id=" + maildata["leaveAppId"] + "&releaseAuth=" + relsauth, true);
