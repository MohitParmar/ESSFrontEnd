...
app.controller('LeavePostingController', function ($scope, $http, $filter) {
    ...
    $scope.GetPostingLeaveApplicationLists = function (data) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                ...
                $scope.lappdata = temparr; $scope.lappdata = $filter('orderBy')($scope.lappdata, '-leaveAppId');
    ...
    $scope.PopulateData = function (id) {
        ...
        req.onreadystatechange = function () { if (req.readyState === 4) { var json = JSON.parse(req.responseText); $scope.lbaldata = json; $scope.$digest(); } };
        req.send();
    ...
    $scope.PerformanceAttendanceRpt = function (data) {
        ...
        if ((typeof (data) === "undefined") || (typeof (data.fdt) === "undefined") || (typeof (data.tdt) === "undefined")) { pr.open('GET', $scope._Conpath + 'Employee/PerfAttd?empunqid=' + str + '&flag=PERF', true); }
        ...
        pr.onreadystatechange = function () {
            if (pr.readyState === 4) {
                ...
                $scope.curPage1 = 0; $scope.pageSize1 = 31;
                $scope.$digest();
            }
        };
        pr.send();
    ...
    $scope.exportAllData = function () {
        setTimeout(function () {
            ...
            var FileName = "Posting_Leave_Application_List_" + d; $scope.JSONToCSVConvertor($scope.InfoPL, FileName, true);
            $('#loading').removeClass("activediv"); $('#loading').addClass("deactivediv");
        }, 100);
    };
