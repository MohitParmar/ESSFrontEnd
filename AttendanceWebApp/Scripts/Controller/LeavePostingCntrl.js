...
app.controller('LeavePostingController', function ($scope, $http, $filter) {
        ...
        function storeTblValues() {
            ...
            for (var i = 0; i < TableData.length; i++) {
                ...
                if (appid == value) {
                    ...
                    //R For Rejection
                    if (value2 === "R") { tbl[count] = { "YearMonth": value1, "LeaveAppId": value, "LeaveAppItem": TableData[i]["LeaveAppItem"], "IsPosted": value2, "Remarks": data.Remarks, "UserId": $('#myEmpUnqId').val() }; count++; }
                    //P For Posting 
                    if (value2 === "P") { tbl[count] = { "YearMonth": value1, "LeaveAppId": value, "LeaveAppItem": TableData[i]["LeaveAppItem"], "IsPosted": TableData[i]["IsPosted"], "UserId": $('#myEmpUnqId').val() }; count++; }
                }
            } return tbl;
        }
    ...
    $scope.GetPostingLeaveApplicationLists = function (data) {
        ...
        if (hidfdt !== '' && hidtdt !== '' && (typeof (data) === "undefined")) { FromDate = hidfdt; ToDate = hidtdt; }
            ...
            var date = new Date();
            var firstDay = new Date(date.getFullYear(), date.getMonth() - 1, 20); var lastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0);
        ...
        var flg = true;
        var xhr = new XMLHttpRequest(); xhr.open('GET', $scope._Conpath + 'LeavePosting/GetLeaves?fromDt=' + FromDate + '&toDt=' + ToDate + '&flag=' + flg, true);
    ...
    $scope.JSONToCSVConvertor = function (JSONData, ReportTitle, ShowLabel) {
        ...
        var fileName = "MyReport_"; fileName += ReportTitle.replace(/ /g, "_"); var uri = 'data:text/csv;charset=utf-8,' + escape(CSV);
