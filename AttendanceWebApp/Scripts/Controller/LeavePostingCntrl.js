...
app.controller('LeavePostingController', function ($scope, $http, $filter) {
    ...
    $scope.currentPage = 1; $scope.itemsPerPage = 25; $scope.alluserlist = []; $scope.popupid = '';
    ...
    $scope.CostLeave = function (data, value, value1, value2) {
        ...
        function storeTblValues() {
            var TableData = new Array(); $('#aliasTable tr').each(function (row, tr) { TableData[row] = { "LeaveAppId": $(tr).find('td:eq(0)').text(), "LeaveAppItem": $(tr).find('td:eq(1)').text(), "IsPosted": $(tr).find("select").val() } });
        ...
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4 && xhr1.status === 200) {
                if (value2 === "R") { var rlsmail = new XMLHttpRequest(); rlsmail.open('GET', $scope._Conpath + 'AutoMail/SendMail?releaseGroupCode=LA&id=' + value, true); rlsmail.setRequestHeader("Content-type", "application/json"); rlsmail.send(); };
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-success alert-dismissable'>" + "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" + "<strong>Leave Posted / Rejected Sucesfully.. </strong>" + "</div>"; $('#MessageBox').show(); TableData = "";
    ...
    $scope.GetPostingLeaveApplicationLists = function (data) {
        ...
        var flg = true;
        var xhr = new XMLHttpRequest(); xhr.open('GET', $scope._Conpath + 'LeavePosting/GetLeaves?fromDt=' + FromDate + '&toDt=' + ToDate + '&flag=' + flg, true); xhr.setRequestHeader('Accept', 'application/json');
                ...
                for (var key in json) {
                    if (json.hasOwnProperty(key)) {
                        for (var key2 in json[key].leaveApplicationDetails) {
                            ...
                            else if (tmpflg === 'C') { if ((tmpcancelled === true && tmpparentId === 0) || (tmpcancelled === false && tmpparentId !== 0)) { temparr[count] = json[key]; count++; break; } };
                        };
                    };
