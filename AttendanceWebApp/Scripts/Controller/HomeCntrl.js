...
app.controller("HomeCntrloller", function ($scope, $http, $filter) {
    ...
    $scope.EmpeMail = function (data) {
        ...
        xhr3.onreadystatechange = function () {
            if (xhr3.readyState === 4 && xhr3.status === 200) {
                ...
                $("#tbl_empdtl").hide();
                $scope.GetUserInfo();
            } else if (xhr3.status === 400 || xhr3.status === 500) {
    ...
    $scope.GetUserInfo = function () {
        ...
        if (e_Code !== "" && typeof e_Code !== "undefined") {
            xhr.open("GET", $scope._Conpath + "Employee/GetEmployee?empunqid=" + e_Code, true);
        } else {
    ...
    $scope.GetUserPerasonalInfo = function () {
        ...
        if (e_Code !== "" && typeof e_Code !== "undefined") {
            per.open("GET", $scope._Conpath + "Employee/GetEmpDetails?empunqid=" + e_Code + "&mode=1", true);
        } else {
    ...
    $scope.GetUserFamInfo = function () {
        var e_Code = $("#eCode").val();
        var fam = new XMLHttpRequest();
        if (e_Code !== "" && typeof e_Code !== "undefined") {
            fam.open("GET", $scope._Conpath + "Employee/GetEmpDetails?empunqid=" + e_Code + "&mode=3", true);
        } else {
            fam.open("GET", $scope._Conpath + "Employee/GetEmpDetails?empunqid=" + $("#myEmpUnqId").val() + "&mode=3", true);
        }
        fam.setRequestHeader("Accept", "application/json");
        fam.onreadystatechange = function () {
            if (fam.readyState === 4) {
                var jsonfam = JSON.parse(fam.responseText);
                $scope.Ufamdata = jsonfam;
                $scope.$digest();
            }
        };
        fam.send();
    ...
    $scope.GetPostedLeave = function () {
        ...
        pstl.open("GET", $scope._Conpath + "leaveentry?empunqid=" + $("#myEmpUnqId").val(), true);
    ...
    $scope.GetPresentAddress = function () {
        ...
        preAdd.onreadystatechange = function () {
            if (preAdd.readyState === 4) {
                ...
                $("#txtHouseNumber").val(arr[0].houseNumber);
                $("#txtSocietyName").val(arr[0].societyName);
                $("#txtAreaName").val(arr[0].areaName);
                $("#txtLandMark").val(arr[0].landMark);
                $("#txtPreCity").val(arr[0].preCity);
                $("#txtTehsil").val(arr[0].tehsil);
                $("#pincode").val(arr[0].prePin);
                $("#txtemailid").val(arr[0].preEmail);
                $("#txtphoneno").val(arr[0].prePhone);
                $("#txtresno").val(arr[0].preResPhone);
                ...
                if (typeof dist === "undefined") {
                    ...
                    $("#txtdist").val(arr[0].preDistrict);
                }
                var state = arr[0].preState;
                if (typeof state === "undefined") {
                    if (l === "BEL") {
                        $("#txtstate").val("KARNATAKA");
                    } else if (l === "KJSAW") {
                        $("#txtstate").val("UTTAR PRADESH");
                    } else {
                        $("#txtstate").val("GUJARAT");
                    }
                } else {
                    $("#txtstate").val(arr[0].preState);
                }
    ...
    $scope.GetSalarySlip = function () {
        var slp = new XMLHttpRequest();
        slp.open("GET", $scope._Conpath + "SalarySlip/GetLinks?empUnqId=" + $("#myEmpUnqId").val(), true);
        slp.setRequestHeader("Accept", "application/json");
        slp.onreadystatechange = function () {
            if (slp.readyState === 4) {
                var json = JSON.parse(slp.responseText);
                $scope.slpdata = json;
                $scope.$digest();
            }
        };
    ...
    $scope.GetSalarySlipDetails = function (yearMonth) {
        ...
        slpdls.open("POST", $scope._Conpath + "SalarySlip/GetSalarySlip", true);
        ...
        slpdls.onload = function () {
            if (slpdls.status == 200) {
                ...
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong> File Not Found...</strong></div>";
                $("#MessageBox").show();
            }
        };
        slpdls.send(jsonobj);
