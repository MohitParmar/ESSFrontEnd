...
app.controller("HomeCntrloller", function ($scope, $http, $filter) {
    ...
    $scope.GetUserFamInfo = function () { var e_Code = $("#eCode").val(), fam = new XMLHttpRequest; "" !== e_Code && "undefined" != typeof e_Code ? fam.open("GET", $scope._Conpath + "Employee/GetEmpDetails?empunqid=" + e_Code + "&mode=3", !0) : fam.open("GET", $scope._Conpath + "Employee/GetEmpDetails?empunqid=" + $("#myEmpUnqId").val() + "&mode=3", !0), fam.setRequestHeader("Accept", "application/json"), fam.onreadystatechange = function () { if (4 === fam.readyState) { var jsonfam = JSON.parse(fam.responseText); $scope.Ufamdata = jsonfam, $scope.$digest() } }, fam.send() }; $scope.GetEmpInfo = function () { $("#tbl_empdtl").show(); var e_Code = $("#eCode").val(); if ("" === e_Code) return document.getElementById("MessageBox").innerHTML = "<div class='alert alert-info alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Please Enter Employee Code First.. </strong></div>", $("#MessageBox").show(), !1; var emp = new XMLHttpRequest; emp.open("GET", $scope._Conpath + "Employee/GetEmployee?empunqid=" + $("#eCode").val(), !0), emp.setRequestHeader("Accept", "application/json"), emp.onreadystatechange = function () { if (4 === emp.readyState) { var json1 = JSON.parse(emp.responseText); $scope.empdata = json1, $scope.$digest() } else 200 !== emp.status && (document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Record Not Found.. </strong></div>", $("#MessageBox").show()) }, emp.send() };
    ...
    $scope.UpdateAddress = function () {
        ...
        societyName = $("#txtSocietyName").val();
        areaName = $("#txtAreaName").val();
        landMark = $("#txtLandMark").val();
        preCity = $("#txtPreCity").val();
        tehsil = $("#txtTehsil").val();
        pincode = $("#pincode").val();
        phoneno = $("#txtphoneno").val();
        PreEmail = $("#txtemailid").val();
        PrePoliceStation = $("#txtPoliceStation").val();
        if (houseNumber === "" || societyName === "" || areaName === "" || landMark === "" || preCity === "" || tehsil === "" || pincode === "" || phoneno === "" || PrePoliceStation === "") {
            alert("Please Fill All Required Details .. ");
            return false;
        }
        var jsonObj = {};
        jsonObj.EmpUnqId = $("#myEmpUnqId").val();
        jsonObj.HouseNumber = houseNumber.toUpperCase();
        jsonObj.SocietyName = societyName.toUpperCase();
        jsonObj.AreaName = areaName.toUpperCase();
        jsonObj.LandMark = landMark.toUpperCase();
        jsonObj.PreCity = preCity.toUpperCase();
        jsonObj.tehsil = tehsil.toUpperCase();
        jsonObj.PoliceStation = PrePoliceStation.toUpperCase();
        ...
        addr.onreadystatechange = function () {
            if (addr.readyState === 4 && addr.status === 200) {
                ...
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" + "<strong>Address / Contact Details are not Saved.As no changes detected.</strong></div>";
                $("#MessageBox").show();
            }
        };
        addr.send(jsonObj);
    };
    $scope.GetPresentAddress = function () { var arr = new Array, preAdd = new XMLHttpRequest; preAdd.open("GET", $scope._Conpath + "EmpAddress/GetEmpAddress?empUnqId=" + $("#myEmpUnqId").val(), !0), preAdd.setRequestHeader("Accept", "application/json"), preAdd.onreadystatechange = function () { if (4 === preAdd.readyState) { var l = $("#myLoc").val(), json = JSON.parse(preAdd.responseText), tmparr = json; arr[0] = { empUnqId: tmparr.empUnqId, houseNumber: tmparr.houseNumber, societyName: tmparr.societyName, areaName: tmparr.areaName, landMark: tmparr.landMark, preCity: tmparr.preCity, tehsil: tmparr.tehsil, preDistrict: tmparr.preDistrict, preState: tmparr.preState, prePin: tmparr.prePin, preEmail: tmparr.preEmail, prePhone: tmparr.prePhone, preResPhone: tmparr.preResPhone, policeStation: tmparr.policeStation }, $("#txtHouseNumber").val(arr[0].houseNumber), $("#txtSocietyName").val(arr[0].societyName), $("#txtAreaName").val(arr[0].areaName), $("#txtLandMark").val(arr[0].landMark), $("#txtPreCity").val(arr[0].preCity), $("#txtTehsil").val(arr[0].tehsil), $("#pincode").val(arr[0].prePin), $("#txtemailid").val(arr[0].preEmail), $("#txtphoneno").val(arr[0].prePhone), $("#txtresno").val(arr[0].preResPhone), $("#txtPoliceStation").val(arr[0].policeStation); var dist = arr[0].preDistrict; "undefined" == typeof dist ? "BEL" === l ? $("#txtdist").val("BELLARY") : "KJSAW" === l ? $("#txtdist").val("MATHURA") : $("#txtdist").val("KUTCH") : $("#txtdist").val(arr[0].preDistrict); var state = arr[0].preState; "undefined" == typeof state ? "BEL" === l ? $("#txtstate").val("KARNATAKA") : "KJSAW" === l ? $("#txtstate").val("UTTAR PRADESH") : $("#txtstate").val("GUJARAT") : $("#txtstate").val(arr[0].preState); $scope.preadddata = arr, $scope.$digest(); } }, preAdd.send() };
    ...
    $scope.FolderUpload = function (syearMonth) {
        ...
        $.ajax({
            type: "POST",
            url: $scope._Conpath + "SalarySlip/UploadPaySlip?folderName=" + syearMonth,
            contentType: false,
            processData: false,
            data: data,
            ...
            error: function (err) {
                $("#loading").removeClass("activediv");
                $("#loading").addClass("deactivediv");
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Files not uploaded Successfully Please Retry...</strong></div>";
                $("#MessageBox").show();
            }
        });
    };
    $scope.PopupPreview = function (odData) {
        ...
        var tdateval, tdate = new Date(oddtl.toDt); tdateval = tdate.getMonth() + 1 < "10" ? tdate.getDate() < "10" ? "0" + tdate.getDate() + "/0" + (tdate.getMonth() + 1) + "/" + tdate.getFullYear() + " " + tdate.getHours() + ":" + tdate.getMinutes() : tdate.getDate() + "/0" + (tdate.getMonth() + 1) + "/" + tdate.getFullYear() + " " + tdate.getHours() + ":" + tdate.getMinutes() : tdate.getDate() < "10" ? "0" + tdate.getDate() + "/" + (tdate.getMonth() + 1) + "/" + tdate.getFullYear() + " " + tdate.getHours() + ":" + tdate.getMinutes() : tdate.getDate() + "/" + (tdate.getMonth() + 1) + "/" + tdate.getFullYear() + " " + tdate.getHours() + ":" + tdate.getMinutes(),
            $("#lblFromDate").text(fdateval), $("#lblToDate").text(tdateval), $("#lblPlaceOfVisit").text(oddtl.placeOfVisit), $("#lblContactDetails").text(oddtl.contactAddress),
