...
app.controller('PendingLeaveCntroller', function ($scope, $http) {
    ...
    //Get All Pending For Post Leave Application List
    $scope.GetPendingForPostLeaveAppList = function (entity) {
        ...
        var rls = new XMLHttpRequest(); rls.open('GET', $scope._Conpath + 'LeavePosting/GetLeaves?fromDt=' + FromDate + '&toDt=' + ToDate, true);
        ...
        rls.onreadystatechange = function () {
            if (rls.readyState === 4) {
                ...
                setTimeout(function () { var d = new Date(); d = d.getFullYear() + "/" + (d.getMonth() + 1) + "/" + d.getDate(); var FileName = "Pending_For_Post_Leave_App_List_" + d; $scope.JSONToCSVConvertor($scope.jsondata, FileName, true); }, 100);
            }
        };
        rls.send();
    };
