angular.module('myApp.Controllers').controller('COffLeaveController', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.ChkHoliday = function () { var d = new Date(); var today; if (d.getMonth() < '10') { today = d.getFullYear() + '-' + '0' + (d.getMonth() + 1) + '-' + d.getDate(); } else { today = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate(); } var FromDate = $("#FromDt option:selected").text(); var fdt = FromDate + " 00:00:00"; var tdt = today + " 00:00:00"; var date1 = new Date(fdt); var date2 = new Date(tdt); if (date2 < date1) { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Can not take COFF before Holiday</strong></div>"; $('#MessageBox').show(); return false; } else { return true; } };//Check Holiday Date with today
    $scope.ToValidate = function () { var FromDate = $("#FromDt option:selected").text(); var chkTo = document.getElementById('ToDt'); var ToDate = chkTo.value; var date1 = new Date(FromDate); var date2 = new Date(ToDate); var diff = ((date1 - date2) / (1000 * 60 * 60 * 24) * -1) + 1; if ($scope.loc === "IPU") { if (diff > 8) { alert("Please Select Correct Date for COff Apply .. "); return false; } } else { if (diff > 91) { alert("Please Select Correct Date for COff Apply .. "); return false; } } $('#TotalDays').text = diff; document.getElementById("TotalDays").value = diff; if (date2 < date1) { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Please Enter Valid Date..</strong></div>"; $('#MessageBox').show(); return false; } else { return true; } };//Date Validation
    var c = 0;
    ...
    $scope.LeaveRequestData = function (entity) {
        ...
        var dtoday = new Date(); var today; if (dtoday.getMonth() < '10') {
            today = dtoday.getFullYear() + '-' + '0' + (dtoday.getMonth() + 1) + '-' + dtoday.getDate();
        } else {
            today = dtoday.getFullYear() + '-' + (dtoday.getMonth() + 1) + '-' + dtoday.getDate();
        }
        var FromDate = $("#FromDt option:selected").text(); var fdt = FromDate + " 00:00:00"; var tdt = today + " 00:00:00";
        var dateholidaydate = new Date(fdt); var datetoday = new Date(tdt); if (datetoday < dateholidaydate) {
            alert("Can not take COFF before Holiday"); return false;
        }
        if ((typeof (entity) === "undefined") || (typeof (entity.ToDt) === "undefined") || (typeof (entity.Remarks) === "undefined") || FromDate === "") {
        ...
        function storeTblValues() {
            var count = 0; c = c + 1; count++; $('.tempRow').remove();
            var row = $("<tr>" +
        ...
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4 && xhr1.status === 200) {
                ...
                var row = $("<tr>" +
                    ...
                    "</tr>");
                $("#aliasTable").append(row); document.getElementById("FromDt").value = ""; document.getElementById("ToDt").value = "";
    ...
    $scope.createLeave = function () {
        ...
        function storeTblValues() {
            ...
            }); TableData.shift(); if (TableData.length > 0) {
                for (var tdata = 0; tdata < TableData.length; tdata++) {
                    var ltyp = TableData[tdata]["leaveTypeCode"]; if (ltyp === '') {
                        return false;
                    }
                }
            } else { return false; }; jsonObj.yearMonth = yearmonth; jsonObj.leaveAppId = 0; jsonObj.empUnqId = $('#myEmpUnqId').val();
            jsonObj.compCode = $('#myCompCode').val(); jsonObj.wrkGrp = $('#myWrkGrp').val(); jsonObj.unitCode = $('#myUnitCode').val();
            jsonObj.deptCode = $('#myDeptCode').val(); jsonObj.statCode = $('#myStatCode').val(); jsonObj.catCode = $('#myCatCode').val();
            jsonObj.isHOD = $('#myIsHod').val(); jsonObj.releaseGroupCode = $('#releaseGroupCode').val(); jsonObj.releaseStrategy = "";
            jsonObj.releaseStatusCode = ""; jsonObj.addDt = dt; jsonObj.addUser = $('#myEmpUnqId').val(); jsonObj.clientIp = $('#myIPAddress').val();
            jsonObj.updDt = dt; jsonObj.updUser = null; jsonObj.remarks = null; jsonObj.parentId = 0; jsonObj.leaveApplicationDetails = TableData;
            return jsonObj;
        };
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 201) {
                ...
                rlsmail.open('GET', $scope._Conpath + 'AutoMail/SendMail?releaseGroupCode=' + maildata["releaseGroupCode"] + '&id=' + maildata["leaveAppId"] +
                    '&releaseAuth=' + relsauth, true); rlsmail.setRequestHeader("Content-type", "application/json"); rlsmail.send();
                //Auto Mail End
                ...
                $('#MessageBox').show(); $("#aliasTable").find("tr:not(:first)").remove(); document.getElementById("FromDt").value = "";
                document.getElementById("ToDt").value = ""; document.getElementById("TotalDays").value = "";
                document.getElementById("Remarks").value = ""; document.getElementById("BtnCreate").disabled = true; document.getElementById("txtLeaveTypeCode").value = "CO";
            } else {
