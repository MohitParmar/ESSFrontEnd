...
app.controller('LeaveReleaseCntrloller', function ($scope, $http, $filter) {
    ...
    var d = new Date(); var releasearr = []; var rlsarr = [];
    ...
    $scope.UpdateLeaveReleaseStatus = function (releaseStatusCode, rlsappid, data) {
        ...
        var xhr2 = new XMLHttpRequest(); xhr2.open('POST', $scope._Conpath + 'AppRelease/UpdateApplReleaseStatus?empUnqId=' + $('#myEmpUnqId').val() + '&releaseStatusCode=' + releaseStatusCode, true); xhr2.setRequestHeader("Content-type", "application/json");
        xhr2.onreadystatechange = function () {
            if (xhr2.readyState === 4) {
                ...
                if (releaseStatusCode === 'F') {
                    ...
                    if (rls_final !== true) { var relsauth = detailarr[ind]["releaseAuth"]; var rlsmail = new XMLHttpRequest(); rlsmail.open('GET', $scope._Conpath + 'AutoMail/SendMail?releaseGroupCode=' + detailarr[ind]["releaseGroupCode"] + '&id=' + rlsappid + '&releaseAuth=' + relsauth, true); rlsmail.setRequestHeader("Content-type", "application/json"); rlsmail.send(); };
                    //Auto Mail End
