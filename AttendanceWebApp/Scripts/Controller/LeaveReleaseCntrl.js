...
app.controller('LeaveReleaseCntrloller', function ($scope, $http, $filter) {
    ...
    $scope.GetLeaveInfo = function () {
        ...
        req.onreadystatechange = function () {
            if (req.readyState === 4) {
                var json = JSON.parse(req.responseText); $scope.reqdata = json;
                $scope.reqdata = $filter('orderBy')($scope.reqdata, '-leaveAppId');
    ...
    $scope.GetLeaveRequestLists = function () {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                var json = JSON.parse(xhr.responseText); rlsarr = json; $scope.data = json;
                $scope.data = $filter('orderBy')($scope.data, '-leaveAppId');
                $scope.curPage1 = 0; $scope.pageSize1 = 10;
                $scope.$digest();
            }
        };
        xhr.send();
            ...
            if (xhr1.readyState === 4) {
                ...
                pstl.onreadystatechange = function () {
                    if (pstl.readyState === 4) {
                        var json = JSON.parse(pstl.responseText); $scope.pladata = json;
                        $scope.pladata = $filter('orderBy')($scope.pladata, '-fromDt');
                        $scope.curPage2 = 0; $scope.pageSize2 = 5;
                        $scope.$digest();
                    }
                };
                pstl.send();
