...
app.controller('MasterCntrloller', function ($scope, $http) {
    ...
    $scope.ListOfholiday = function () {
        ...
        hld.onreadystatechange = function () {
            if (hld.readyState === 4) {
                var json = JSON.parse(hld.responseText);
                $scope.hlddata = json;
                $scope.$digest();
            }
        };
    ...
    $scope.LoadDiv = function (data) {
        if (data === "LR") {
            ...
            $('#div_LeaveApplication').hide(); $('#div_UpdateEmailId').hide(); $('#div_ReleaseStrategy').hide(); $('#div_LeaveCancellation').hide();
            $('#div_LeavePosting').hide();
        };
        if (data === "LA") {
            $('#div_LeaveApplication').show();
            $('#div_LeaveRelease').hide(); $('#div_UpdateEmailId').hide(); $('#div_ReleaseStrategy').hide(); $('#div_LeaveCancellation').hide();
            $('#div_LeavePosting').hide();
        };
        if (data === "EM") {
            $('#div_UpdateEmailId').show();
            $('#div_LeaveRelease').hide(); $('#div_LeaveApplication').hide(); $('#div_ReleaseStrategy').hide(); $('#div_LeaveCancellation').hide();
            $('#div_LeavePosting').hide();
        };
        if (data === "RS") {
            $('#div_ReleaseStrategy').show();
            $('#div_LeaveRelease').hide(); $('#div_LeaveApplication').hide(); $('#div_UpdateEmailId').hide(); $('#div_LeaveCancellation').hide();
            $('#div_LeavePosting').hide();
        };
        if (data === "LC") {
            $('#div_LeaveCancellation').show();
            $('#div_LeaveRelease').hide(); $('#div_LeaveApplication').hide(); $('#div_UpdateEmailId').hide(); $('#div_ReleaseStrategy').hide();
            $('#div_LeavePosting').hide();
        };
        if (data === "LP") {
            $('#div_LeavePosting').show();
            $('#div_LeaveRelease').hide(); $('#div_LeaveApplication').hide(); $('#div_UpdateEmailId').hide(); $('#div_ReleaseStrategy').hide();
            $('#div_LeaveCancellation').hide();
        };
    };
});