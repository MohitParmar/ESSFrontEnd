...
app.controller('LeavePerformanceReportController', function ($scope, $http) {
    ...
    $scope.GetLeavePerformanceReport = function (dt) {
        ...
        var FromDate = dt.FromDt; var ToDate = dt.ToDt; var date1 = new Date(FromDate); var date2 = new Date(ToDate);
        ...
        Perf.onreadystatechange = function () {
            if (Perf.readyState === 4) {
                ...
                var json = JSON.parse(Perf.responseText); $scope.perfdata = json; $scope.lPerRpt = json;
                var test = $scope.perfdata; var srtcv = test[0];
                var arr = [];
