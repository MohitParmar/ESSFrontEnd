...
app.controller('LeavePerformanceReportController', function ($scope, $http) {
    ...
    $scope.GetLeavePerformanceReport = function (dt) {
        ...
        var Perf = new XMLHttpRequest();
        Perf.open('GET', $scope._Conpath + 'LeaveReport/GetLeaves?fromDt=' + FromDate + '&toDt=' + ToDate, true);
        ...
        Perf.onreadystatechange = function () {
            if (Perf.readyState === 4) {
                ...
                var json = JSON.parse(Perf.responseText);
                $scope.perfdata = json;
                $scope.lPerRpt = json;

                var test = $scope.perfdata; //STORE JSON DATA IN ARRAY 
                var srtcv = test[0];    //GET FIRST ROW DATA OF TEST ARRAY
                var arr = [];

                //Get Columns Names using key & Store in Array
                for (var key in srtcv) { var s = key; arr.push(key); }   //Replace Column Name

                var tmparr1 = new Array(arr);

                $scope.headers = arr;
                
                var tmparr = new Array(arr.length);
                for (var i = 0; i < arr.length; i++) {
                    tmparr[i] = new Array(test.length);
                }

                for (var i = 0; i < test.length; i++) {
                    for (var j = 0; j < arr.length; j++) {
                        var colname = arr[j];
                        var test0 = test[i][colname];
                        tmparr[j][i] = test0;
                    }
                }

                var transposedArray = transpose(tmparr);
                $scope.dta = transposedArray;
                $scope.lPerRpt = tmparr1.concat(transposedArray);
                $scope.curPage = 0;
                $scope.pageSize = 25;
                $scope.$digest();
            }
    ...
    $scope.exportAllData = function () {
        setTimeout(function () {
            ...
            var FileName = "Leave_Performance_List_" + d;

            $scope.JSONToCSVConvertor($scope.lPerRpt, FileName, true);
