...
app.controller('LeavePerformanceReportController', function ($scope, $http) {
    ...
    $scope.GetLeavePerformanceReport = function (dt) {
        ...
        var FromDate = dt.FromDt; var ToDate = dt.ToDt;
        var date1 = new Date(FromDate); var date2 = new Date(ToDate);
        ...
        Perf.onreadystatechange = function () {
            if (Perf.readyState === 4) {
                ...
                $scope.perfdata = json; $scope.lPerRpt = json;
                ...
                $scope.curPage = 0; $scope.pageSize = 25;
                $scope.$digest();
            }
        };
        Perf.send();
    ...
    $scope.exportAllData = function () {
        setTimeout(function () {
            ...
            var FileName = "Leave_Performance_List_" + d; $scope.JSONToCSVConvertor($scope.lPerRpt, FileName, true);
            $('#loading').removeClass("activediv"); $('#loading').addClass("deactivediv");
        }, 100);
    };
