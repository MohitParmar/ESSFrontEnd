...
app.controller('MediclaimCntroller', function ($scope, $http, $filter) {
    ...
    $scope._calculateAge = function () {
        var birthday = document.getElementById('dtDOB').value; var d = new Date(); var today = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate(); var date1 = new Date(birthday); var date2 = new Date(today);
        var yearCount = diff_years(date1, date2); $('#txtAge').val(yearCount);
        function diff_years(dt2, dt1) { var diff = (dt2.getTime() - dt1.getTime()) / 1000; diff /= (60 * 60 * 24); return Math.abs(Math.round(diff / 365.25)); }
    }
    ...
    $scope.GetEmpInfo = function () {
        ...
        emp.onreadystatechange = function () {
            if (emp.readyState === 4) { var json = JSON.parse(emp.responseText); $scope.empdata = json; $scope.$digest(); }
