    ...
    $scope.GetRelesaseStratey = function () {
        ...
        if (tyear < fyear || fyear < tyear) {
            document.getElementById("MessageBox").innerHTML =
                ...
                "<strong>Please Enter Valid Date of Current Month/Year.. </strong></div>"; $('#MessageBox').show();
            document.getElementById("PresentWODt").value = ""; return false;
        } else { $scope.ToValidate(); };
    }; $scope.ToValidate = function () {
        var chkFrom = document.getElementById('PresentWODt'); var chkTo = document.getElementById('ChangeWODt');
        var FromDate = chkFrom.value; var ToDate = chkTo.value;
        if (FromDate === "") {
            document.getElementById("MessageBox").innerHTML =
                "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                "<strong>Please Select From Date First...</strong></div>"; $('#MessageBox').show();
            document.getElementById("PresentWODt").value = ""; document.getElementById("ChangeWODt").value = ""; return false;
        };
        var date1 = new Date(FromDate); var date2 = new Date(ToDate); var fyear = date2.getFullYear(); var d = new Date(); var tyear = d.getFullYear();
        if (tyear < fyear) { alert("Please Enter Valid Date of Current Month/Year.. "); document.getElementById("ChangeWODt").value = ""; return false; };
        var diff = ((date1 - date2) / (1000 * 60 * 60 * 24) * -1) + 1;
        if (diff === "NaN" || diff <= 0) {
            document.getElementById("MessageBox").innerHTML =
                "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                "<strong>Please Select Proper Dates for OD apply.</strong></div>"; $('#MessageBox').show();
            document.getElementById("PresentWODt").value = ""; document.getElementById("ChangeWODt").value = ""; return false;
        };
        if (date2 < date1) {
            document.getElementById("MessageBox").innerHTML =
        ...
        var FromDateTime = entity.PresentWODt; var ToDateTime = entity.ChangeWODt;
        ...
        xhr.setRequestHeader("Content-type", "application/json"); xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 201) {
                document.getElementById("MessageBox").innerHTML =
                    ...
                    "<strong>Week Off Change Application Created Sucessfully.. </strong></div>"; $('#MessageBox').show();
                document.getElementById("PresentWODt").value = ""; document.getElementById("ChangeWODt").value = "";
            } else {
                document.getElementById("MessageBox").innerHTML =
                    "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                    "<strong>Wekk Off Change Application Not Created Please try Again.. </strong></div>"; $('#MessageBox').show();
                document.getElementById("PresentWODt").value = ""; document.getElementById("ChangeWODt").value = "";
            };
        }; xhr.send(TableData);
    };
