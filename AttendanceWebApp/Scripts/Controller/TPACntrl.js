...
app.controller("TPAController", function ($scope, $http, $filter) {
    ...
    $scope.exportObj; $scope.bulkSanExportObj;
    ...
    $scope.GetPreTpa = function (dtFlg) {
        document.getElementById("btnSearch").disabled = true; $scope.tpaData = ""; $('#loading').removeClass("deactivediv"); $('#loading').addClass("activediv");
        var fromDate, toDate; fromDate = $("#fromDt").val(); toDate = $("#toDt").val(); if (dtFlg) {
            if (fromDate === "" || toDate === "") { alert("Please Select From and to Date."); return false; };
        } else { if (fromDate === "") { alert("Please Select TPA Date."); return false; } else { toDate = fromDate; }; var tpaDate = new Date(fromDate); tpaDate = tpaDate.toISOString().split('T')[0]; var now = new Date(); now = now.toISOString().split('T')[0]; if (tpaDate < now) { alert("Back Date Entry not allowed."); document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'>" + "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" + "<strong>Back Date Entry not allowed.</strong>" + "</div>"; $('#MessageBox').show(); }; };
        var tpa = new XMLHttpRequest();
        tpa.open("GET", $scope._Conpath + "Tpa/getpretpa?fromDate=" + fromDate + "&toDate=" + toDate + "&empUnqId=" + $("#myEmpUnqId").val(), true);
            ...
            if (tpa.readyState === 4 && tpa.status === 200) {
                ...
                if (dtFlg) {
                    ...
                    for (var e = 0; e < result.length; e++) {
                        ...
                        myArray.push([]);
                        myArray[cnt].empUnqId = empRowArr[0].empUnqId; myArray[cnt].empName = empRowArr[0].empName;
                        row += "<tr><td>" + empRowArr[0].empUnqId + "</td>" +
                            "<td>" + empRowArr[0].empName + "</td><td><input id='txtJustification' style='width:100px;' class='form-control'></td>";
                        //EMP DATE LOOP
                        for (var p = 0; p < empRowArr.length; p++) {
                            var loopDate = new Date(empRowArr[p].tpaDate);
                            var MyDateString = ('0' + loopDate.getDate()).slice(-2); myArray[cnt][MyDateString] = "";
                            var g = cnt + MyDateString;
                            var f = empRowArr[p].id;
                            var rhours = empRowArr[p].requiredHours;
                            if (f !== 0) {
                                row += "<td><input id='" + g + "' style='width:45px;' class='form-control' value=" + rhours +
                                    " disabled><input type='hidden' id='hidId" + g + "' value=" + f + "></td>";
                            }
                            else {
                                row += "<td><input id='" + g + "' style='width:45px;' class='form-control'><input type='hidden' id='hidId" + g +
                                    "' value='0'></td>";
                            };
                        };
                        row += "</tr>";
    ...
    $scope.PreTpaRequest = function (dtFlg) {
        document.getElementById("btnOK").disabled = true; var fromDate, toDate; fromDate = $("#fromDt").val(); toDate = $("#toDt").val(); var fdate = new Date(fromDate); fdate = fdate.toISOString().split('T')[0];
        var TableData = storeTblValues();
        function storeTblValues() {
            var TableData = new Array(); var ind = 0, cnt = 0;
            if (dtFlg) {
                $('#tblTPAData tr').each(function (row, tr) {
                    for (var loopDate = new Date(fromDate); loopDate <= new Date(toDate); loopDate.setDate(loopDate.getDate() + 1)) {
                        var tpaDate = loopDate.toISOString().split('T')[0]; var MyDateString = ('0' + loopDate.getDate()).slice(-2); var f = cnt + MyDateString;
                        TableData[ind] = { "id": 0, "empUnqId": $(tr).find('td:eq(0)').text(), "tpaDate": tpaDate, "tpaShiftCode": '', "requiredHours": $("#" + f).val(), "hidId": $("#hidId" + f).val(), "preJustification": $(tr).find("td:eq(2) input").val(), "releaseGroupCode": "OT", "releaseStrategy": $(tr).find('td:eq(0)').text(), "preReleaseStatusCode": "I", "preRemarks": null, "addDt": now, "addUser": $("#myEmpUnqId").val() }
                        ind++;
                    } cnt++;
                });
            } else { $('#tblTPAData tr').each(function (row, tr) { TableData[row] = { "id": 0, "empUnqId": $(tr).find('td:eq(1)').text(), "tpaDate": fdate, "tpaShiftCode": $(tr).find('td:eq(6)').text(), "requiredHours": $(tr).find("td:eq(7) input").val(), "preJustification": $(tr).find("td:eq(8) input").val(), "releaseGroupCode": "OT", "releaseStrategy": $(tr).find('td:eq(1)').text(), "preReleaseStatusCode": "I", "preRemarks": null, "addDt": now, "addUser": $("#myEmpUnqId").val() } }); }
            var jsonObj = new Array(); var cnt = 0;
            for (var i = 0; i < TableData.length; i++) {
                if (TableData[i].empUnqId !== "") {
                    if (TableData[i].requiredHours !== "" && (typeof (TableData[i].requiredHours) !== "undefined") &&
                        parseInt(TableData[i].requiredHours) !== 0 && parseInt(TableData[i].requiredHours) > 1) {
                        if (dtFlg) { if (parseInt(TableData[i].hidId) === 0) { jsonObj[cnt] = TableData[i]; cnt++; } } else { jsonObj[cnt] = TableData[i]; cnt++; }
                    };
                };
            }; return jsonObj;
        };
        var e = ""; for (var i = 0; i < TableData.length; i++) { if (TableData[i].preJustification === "") { e += "Justification required for this Employee " + TableData[i].empUnqId + " TPA."; document.getElementById("btnOK").disabled = false; } }
        if (e !== "") { alert("Justification Required."); document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'>" + "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" + "<strong>" + e + "</strong></div>"; $('#MessageBox').show(); document.getElementById("btnOK").disabled = false; return false; }
        TableData = JSON.stringify(TableData);
        var xhr1 = new XMLHttpRequest(); xhr1.open('POST', $scope._Conpath + 'Tpa/pretparequest?empUnqId=' + $("#myEmpUnqId").val(), true); xhr1.setRequestHeader("Content-type", "application/json"); xhr1.onreadystatechange = function () {
    ...
    $scope.PreApproval = function () {
        document.getElementById("btnOK").disabled = true; var preDataArr = new Array(); preDataArr = $scope.preData; var TableData = storeTblValues();
        ...
        for (var i = 0; i < TableData.length; i++) { var chkReleased = TableData[i].preReleaseStatusCode; var preRemarks = TableData[i].preRemarks; if (chkReleased === false && preRemarks === "") { alert("Remarks is mandatory if release check removed."); return false; } };   //Check Release Flag
        var approvedTPAArr = new Array(); var cnt = 0;
        for (var j = 0; j < preDataArr.length; j++) { var tpaRelStatus; tpaRelStatus = preDataArr[j].tpaReleaseStatus; for (var k = 0; k < TableData.length; k++) { if (parseInt(TableData[k].id) === tpaRelStatus[0].tpaSanctionId) { approvedTPAArr[cnt] = tpaRelStatus[0]; if (TableData[k].preReleaseStatusCode === true) { approvedTPAArr[cnt].preReleaseStatusCode = "F"; } else { approvedTPAArr[cnt].preReleaseStatusCode = "R"; }; approvedTPAArr[cnt].preReleaseDate = now; approvedTPAArr[cnt].preRemarks = TableData[k].preRemarks; cnt++; break; } } }
        var jsonObj = JSON.stringify(approvedTPAArr);
        var xhr1 = new XMLHttpRequest(); xhr1.open('POST', $scope._Conpath + 'Tpa/preapproval', true); xhr1.setRequestHeader("Content-type", "application/json"); xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4 && xhr1.status === 200) {
                ...
                document.getElementById("btnOK").disabled = false; TableData = ""; jsonObj = "";
    ...
    $scope.SanctionTpa = function () {
        ...
        var jsonObj = new Array(); var cnt = 0;
        for (var i = 0; i < TableData.length; i++) { if (TableData[i].sanctionTpa >= 2) { jsonObj[cnt] = TableData[i]; cnt++; }; };
        for (var j = 0; j < jsonObj.length; j++) {
            var requiredTPA = parseInt(jsonObj[j].requiredHours); var sanctionTPA = parseInt(jsonObj[j].sanctionTpa); var calcOverTime = parseInt(jsonObj[j].calcOverTime); var empUnqId = jsonObj[j].empUnqId; var tpaDate = jsonObj[j].tpaDate; var postJustification = jsonObj[j].postJustification;
            if (sanctionTPA > calcOverTime) {
                document.getElementById("btnOK").disabled = false;
                alert("Sanction TPA is not allow to greater of actual tpa hours for this Employee " + empUnqId + " TPA Date " + tpaDate); return false; break;
            };
        ...
        var san = new XMLHttpRequest(); san.open('POST', $scope._Conpath + 'Tpa/sanctiontpa?empUnqId=' + $("#myEmpUnqId").val(), true); san.setRequestHeader("Content-type", "application/json"); san.onreadystatechange = function () {
    ...
    $scope.posData; $scope.GetSanctionList = function () {
        ...
        var pos = new XMLHttpRequest(); pos.open("GET", $scope._Conpath + "Tpa/getsanctionlist?empUnqId=" + $("#myEmpUnqId").val(), true); pos.setRequestHeader("Accept", "application/json"); pos.onreadystatechange = function () {
            if (pos.readyState === 4 && pos.status === 200) { $('#loading').removeClass("activediv"); $('#loading').addClass("deactivediv"); var json = JSON.parse(pos.responseText); $scope.posData = json; $scope.$digest(); }
    ...
    $scope.PostApproval = function () {
        document.getElementById("btnOK").disabled = true;
        var postDataArr = new Array(); postDataArr = $scope.posData; var TableData = storeTblValues();
        function storeTblValues() {
            var TableData = new Array(); $('#tblTPAData tr').each(function (row, tr) {
                TableData[row] = { "id": $(tr).find('td:eq(0)').text(), "empUnqId": $(tr).find('td:eq(2)').text(), "tpaDate": $(tr).find('td:eq(1)').text(), "postReleaseStatusCode": $(tr).find('td:eq(11) input').prop("checked"), "postRemarks": $(tr).find('td:eq(12) input').val() }
            }); TableData.shift(); TableData.shift(); TableData.splice(-1); return TableData;
        };
        for (var i = 0; i < TableData.length; i++) {
            var chkReleased = TableData[i].postReleaseStatusCode; var postRemarks = TableData[i].postRemarks;
            if (chkReleased === false && postRemarks === "") { document.getElementById("btnOK").disabled = false; alert("Remarks is mandatory if release check removed."); return false; }
        }; //Check Release Flag
        var approvedTPAArr = new Array(); var cnt = 0;
        ...
        var xhr2 = new XMLHttpRequest(); xhr2.open('POST', $scope._Conpath + 'Tpa/postapproval', true); xhr2.setRequestHeader("Content-type", "application/json"); xhr2.onreadystatechange = function () {
            if (xhr2.readyState === 4 && xhr2.status === 200) {
                document.getElementById("btnOK").disabled = false;
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" + "<strong>Approved..</strong>" + "</div>"; $('#MessageBox').show(); TableData = ""; jsonObj = "";
                $scope.GetSanctionList();
            }
    ...
    $scope.phrData; $scope.PendingSanctionListHR = function () {
        ...
        phr.open("GET", $scope._Conpath + "Tpa/gethrlist", true);
        phr.setRequestHeader("Accept", "application/json");
        phr.onreadystatechange = function () {
            if (phr.readyState === 4 && phr.status === 200) {
                ...
                var json = JSON.parse(phr.responseText);
                $scope.phrData = json;
                $scope.$digest();
            }
            else if (xhr2.status === 400 || xhr2.status === 403 || xhr2.status === 404 || xhr2.status === 408 || xhr2.status === 500) {
                $('#loading').removeClass("activediv");
                $('#loading').addClass("deactivediv");
                var str = phr.responseText.replace("[", '').replace("]", '').replace("{", '').replace("}", '').toString();
    ...
    $scope.PostApprovalHR = function () {
        ...
        function storeTblValues() {
            ...
            $('#tblTPAData tr').each(function (row, tr) {
                TableData[row] = {
                    ...
                    "empUnqId": $(tr).find('td:eq(2)').text(),
                    "tpaDate": $(tr).find('td:eq(1)').text(),
                    "hrReleaseStatusCode": $(tr).find('td:eq(11) input').prop("checked"),
                    "hrPostRemarks": $(tr).find('td:eq(12) input').val()
                }
            });
            TableData.shift(); TableData.shift(); TableData.splice(-1);
            return TableData;
        };
        for (var i = 0; i < TableData.length; i++) {
            var chkReleased = TableData[i].hrReleaseStatusCode;
            var postRemarks = TableData[i].hrPostRemarks;
            if (chkReleased === false && postRemarks === "") {
                document.getElementById("btnOK").disabled = false;
                alert("Remarks is mandatory if release check removed.");
                return false;
            }
        }; //Check Release Flag
        for (var j = 0; j < postDataArr.length; j++) {
            var tpaRelStatus;
            tpaRelStatus = postDataArr[j].tpaReleaseStatus;
            for (var k = 0; k < TableData.length; k++) {
                if (parseInt(TableData[k].id) === tpaRelStatus[0].tpaSanctionId) {
                    if (TableData[k].hrReleaseStatusCode === true) {
                        postDataArr[j].hrReleaseStatusCode = "F";
                    } else {
                        postDataArr[j].hrReleaseStatusCode = "R";
                    };
                    postDataArr[j].hrReleaseDate = now;
                    postDataArr[j].hrPostRemarks = TableData[k].hrPostRemarks;
                    postDataArr[j].hrUser = $("#myEmpUnqId").val();
                    break;
                }
            }
        };
        var jsonObj = JSON.stringify(postDataArr);
        var HRP = new XMLHttpRequest();
        HRP.open('POST', $scope._Conpath + 'Tpa/posthr', true);
        HRP.setRequestHeader("Content-type", "application/json");
        HRP.onreadystatechange = function () {
            if (HRP.readyState === 4 && HRP.status === 200) {
                ...
                TableData = "";
                jsonObj = "";
                $scope.PendingSanctionListHR();
            }
            else if (HRP.status === 400 || HRP.status === 403 || HRP.status === 404 || HRP.status === 408 || HRP.status === 500) {
                document.getElementById("btnOK").disabled = false;
                var str = HRP.responseText.replace("[", '').replace("]", '').toString();
                var fields = str.split(',');
                var er = "";
                for (var i = 0; i < fields.length; i++) {
                    er = er + fields[i] + "<br/>";
        ...
        var san = new XMLHttpRequest(); san.open("GET", $scope._Conpath + "Tpa/sanctionreport?fromDate=" + fromDt + "&toDate=" + toDt, true); san.setRequestHeader("Accept", "application/json"); san.onreadystatechange = function () {
            if (san.readyState === 4 && san.status === 200) {
                ...
                for (var l = 0; l < sanArr.length; l++) {
                    ...
                    if (l === 0) { myArray[l]["AppDate"] = ""; myArray[l]["EmpUnqID"] = ""; myArray[l]["EmpName"] = ""; myArray[l]["Department"] = ""; myArray[l]["Station"] = ""; myArray[l]["Category"] = ""; myArray[l]["Designation"] = ""; myArray[l]["SanDate"] = ""; myArray[l]["InTime"] = ""; myArray[l]["OutTime"] = ""; myArray[l]["ShiftCode"] = ""; myArray[l]["TPAHours"] = ""; myArray[l]["Reason"] = ""; myArray[l]["Status"] = ""; myArray[l]["AttdStatus"] = ""; myArray[l]["1_ReleaseLevel"] = ""; myArray[l]["1_PreReleaser"] = ""; myArray[l]["1_PreReleaseDate"] = ""; myArray[l]["1_SanctionReleaser"] = ""; myArray[l]["1_SanctionReleaseDate"] = ""; myArray[l]["2_ReleaseLevel"] = ""; myArray[l]["2_PreReleaser"] = ""; myArray[l]["2_PreReleaseDate"] = ""; myArray[l]["2_SanctionReleaser"] = ""; myArray[l]["2_SanctionReleaseDate"] = ""; myArray[l]["3_ReleaseLevel"] = ""; myArray[l]["3_PreReleaser"] = ""; myArray[l]["3_PreReleaseDate"] = ""; myArray[l]["3_SanctionReleaser"] = ""; myArray[l]["3_SanctionReleaseDate"] = ""; };
                    myArray[l]["AppDate"] = sanArr[l].addDt.substring(0, sanArr[l].addDt.indexOf("T"));
                    myArray[l]["EmpUnqID"] = sanArr[l].empUnqId; bulkSanctionArray[l]["EmpUnqID"] = sanArr[l].empUnqId;
                    myArray[l]["EmpName"] = sanArr[l].empName; myArray[l]["Department"] = sanArr[l].deptName; myArray[l]["Station"] = sanArr[l].statName; myArray[l]["Category"] = sanArr[l].catName; myArray[l]["Designation"] = sanArr[l].desgName;
                    myArray[l]["SanDate"] = sanArr[l].tpaDate.substring(0, sanArr[l].tpaDate.indexOf("T")); bulkSanctionArray[l]["SanDate"] = sanArr[l].tpaDate.substring(0, sanArr[l].tpaDate.indexOf("T"));
                    myArray[l]["InTime"] = sanArr[l].consIn; bulkSanctionArray[l]["InTime"] = sanArr[l].consIn;
                    myArray[l]["OutTime"] = sanArr[l].consOut; bulkSanctionArray[l]["OutTime"] = sanArr[l].consOut;
                    myArray[l]["ShiftCode"] = sanArr[l].actShiftCode; bulkSanctionArray[l]["ShiftCode"] = sanArr[l].actShiftCode;
                    myArray[l]["TPAHours"] = sanArr[l].sanctionTpa; bulkSanctionArray[l]["TPAHours"] = sanArr[l].sanctionTpa;
                    ...
                    if (Reason === null || Reason === "") {
                        ...
                        myArray[l]["Reason"] = sanArr[l].preJustification.replace(/[^a-zA-Z0-9 ]/g, "").toUpperCase();
                        bulkSanctionArray[l]["Reason"] = sanArr[l].preJustification.replace(/[^a-zA-Z0-9 ]/g, "").toUpperCase();
                    };
                    myArray[l]["Status"] = sanArr[l].status; bulkSanctionArray[l]["Status"] = sanArr[l].status;
                    myArray[l]["AttdStatus"] = sanArr[l].leaveType; bulkSanctionArray[l]["AttdStatus"] = sanArr[l].leaveType;
                    bulkSanctionArray[l]["Station"] = sanArr[l].statName;
                    for (var i = 0; i < appreleasearr.length; i++) {
                        var r_preReleaseDate, r_postReleaseDate, isFinalRelease;
                        r_preReleaseDate = appreleasearr[i].preReleaseDate; if (r_preReleaseDate === null) { r_preReleaseDate = ""; } else { r_preReleaseDate = appreleasearr[i].preReleaseDate.substring(0, appreleasearr[i].preReleaseDate.indexOf("T")); };
                        r_postReleaseDate = appreleasearr[i].postReleaseDate; if (r_postReleaseDate === null) { r_postReleaseDate = ""; } else { r_postReleaseDate = appreleasearr[i].postReleaseDate.substring(0, appreleasearr[i].postReleaseDate.indexOf("T")); };
                        if (i === 0) { myArray[l]["1_ReleaseLevel"] = appreleasearr[i].releaseStrategyLevel; myArray[l]["1_PreReleaser"] = appreleasearr[i].preReleaseAuth; myArray[l]["1_PreReleaseDate"] = r_preReleaseDate; myArray[l]["1_SanctionReleaser"] = appreleasearr[i].postReleaseAuth; myArray[l]["1_SanctionReleaseDate"] = r_postReleaseDate; }
                        else if (i === 1) { myArray[l]["2_ReleaseLevel"] = appreleasearr[i].releaseStrategyLevel; myArray[l]["2_PreReleaser"] = appreleasearr[i].preReleaseAuth; myArray[l]["2_PreReleaseDate"] = r_preReleaseDate; myArray[l]["2_SanctionReleaser"] = appreleasearr[i].postReleaseAuth; myArray[l]["2_SanctionReleaseDate"] = r_postReleaseDate; }
                        else if (i === 2) { myArray[l]["3_ReleaseLevel"] = appreleasearr[i].releaseStrategyLevel; myArray[l]["3_PreReleaser"] = appreleasearr[i].preReleaseAuth; myArray[l]["3_PreReleaseDate"] = r_preReleaseDate; myArray[l]["3_SanctionReleaser"] = appreleasearr[i].postReleaseAuth; myArray[l]["3_SanctionReleaseDate"] = r_postReleaseDate; };
                        isFinalRelease = appreleasearr[i].isFinalRelease; if (isFinalRelease) { bulkSanctionArray[l]["SanctionTPAReleaseDate"] = r_postReleaseDate; };
                    };
                };
                $scope.sanObj = myArray; $scope.sanObj = $filter('orderBy')($scope.sanObj, 'SanDate'); $scope.exportObj = $scope.sanObj;
                $scope.bulkSanObj = bulkSanctionArray; $scope.bulkSanObj = $filter('orderBy')($scope.bulkSanObj, 'SanDate'); $scope.bulkSanExportObj = $scope.bulkSanObj;
                $scope.$digest(); $('#loading').removeClass("activediv"); $('#loading').addClass("deactivediv"); document.getElementById("btnSearch").disabled = false;
            }
            else if (xhr2.status === 400 || xhr2.status === 403 || xhr2.status === 404 || xhr2.status === 408 || xhr2.status === 500) {
    ...
    $scope.TpaReport = function () {
        $('#loading').removeClass("deactivediv"); $('#loading').addClass("activediv"); var fromDt = $("#fromDt").val(); var toDt = $("#toDt").val(); var san = new XMLHttpRequest(); san.open("GET", $scope._Conpath + "Tpa/TpaReport?fromDate=" + fromDt + "&toDate=" + toDt + "&empUnqId=" + $("#myEmpUnqId").val(), true); san.setRequestHeader("Accept", "application/json"); san.onreadystatechange = function () {
            if (san.readyState === 4 && san.status === 200) {
                $('#loading').removeClass("activediv"); $('#loading').addClass("deactivediv");
                var json = JSON.parse(san.responseText); $scope.relTPAData = json; $scope.$digest();
                var sanArr = json; var myArray = []; var appreleasearr = new Array();
                for (var l = 0; l < sanArr.length; l++) {
                    appreleasearr = sanArr[l].tpaReleaseStatus; myArray.push([]);
                    myArray[l]["AppDate"] = sanArr[l].addDt.substring(0, sanArr[l].addDt.indexOf("T")); myArray[l]["EmpUnqID"] = sanArr[l].empUnqId; myArray[l]["EmpName"] = sanArr[l].empName; myArray[l]["Station"] = sanArr[l].statName; myArray[l]["Category"] = sanArr[l].catName; myArray[l]["Designation"] = sanArr[l].desgName;
                    myArray[l]["TPADate"] = sanArr[l].tpaDate.substring(0, sanArr[l].tpaDate.indexOf("T"));
                    var actShiftCode = sanArr[l].actShiftCode; if (actShiftCode === null) { myArray[l]["ActualShift"] = ""; } else { myArray[l]["ActualShift"] = actShiftCode; };
                    myArray[l]["RequiredHours"] = sanArr[l].requiredHours; myArray[l]["PreJustification"] = sanArr[l].preJustification; myArray[l]["PreReleaseStatus"] = sanArr[l].preReleaseStatusCode; myArray[l]["SanctionTpa"] = sanArr[l].sanctionTpa;
                    var postJustification = sanArr[l].postJustification; if (postJustification === null) { myArray[l]["SanctionJustification"] = ""; } else { myArray[l]["SanctionJustification"] = postJustification; };
                    var postReleaseStatusCode = sanArr[l].postReleaseStatusCode; if (postReleaseStatusCode === null) { myArray[l]["SanctionReleaseStatus"] = ""; } else { myArray[l]["SanctionReleaseStatus"] = postReleaseStatusCode; };
                    for (var i = 0; i < appreleasearr.length; i++) {
                        var r_preReleaseDate, r_postReleaseDate, r_postReleaseStatusCode = "";
                        r_preReleaseDate = appreleasearr[i].preReleaseDate; if (r_preReleaseDate === null) { r_preReleaseDate = ""; } else { r_preReleaseDate = appreleasearr[i].preReleaseDate.substring(0, appreleasearr[i].preReleaseDate.indexOf("T")); };
                        r_postReleaseDate = appreleasearr[i].postReleaseDate; if (r_postReleaseDate === null) { r_postReleaseDate = ""; } else { r_postReleaseDate = appreleasearr[i].postReleaseDate.substring(0, appreleasearr[i].postReleaseDate.indexOf("T")); };
                        r_postReleaseStatusCode = appreleasearr[i].postReleaseStatusCode; if (r_postReleaseStatusCode === null) { r_postReleaseStatusCode = ""; };
                        if (i === 0) { myArray[l]["1_ReleaseLevel"] = appreleasearr[i].releaseStrategyLevel; myArray[l]["1_PreReleaser"] = appreleasearr[i].preReleaseAuth; myArray[l]["1_PreReleaserName"] = appreleasearr[i].preReleaseName; myArray[l]["1_PreReleaseDate"] = r_preReleaseDate; myArray[l]["1_PreReleaseStatus"] = appreleasearr[i].preReleaseStatusCode; myArray[l]["1_SanctionReleaser"] = appreleasearr[i].postReleaseAuth; myArray[l]["1_SanctionReleaserName"] = appreleasearr[i].postReleaseName; myArray[l]["1_SanctionReleaseDate"] = r_postReleaseDate; myArray[l]["1_SanctionReleaseStatus"] = r_postReleaseStatusCode; }
                        else if (i === 1) { myArray[l]["2_ReleaseLevel"] = appreleasearr[i].releaseStrategyLevel; myArray[l]["2_PreReleaser"] = appreleasearr[i].preReleaseAuth; myArray[l]["2_PreReleaserName"] = appreleasearr[i].preReleaseName; myArray[l]["2_PreReleaseDate"] = r_preReleaseDate; myArray[l]["2_PreReleaseStatus"] = appreleasearr[i].preReleaseStatusCode; myArray[l]["2_SanctionReleaser"] = appreleasearr[i].postReleaseAuth; myArray[l]["2_SanctionReleaserName"] = appreleasearr[i].postReleaseName; myArray[l]["2_SanctionReleaseDate"] = r_postReleaseDate; myArray[l]["2_SanctionReleaseStatus"] = r_postReleaseStatusCode; }
                        else if (i === 2) { myArray[l]["3_ReleaseLevel"] = appreleasearr[i].releaseStrategyLevel; myArray[l]["3_PreReleaser"] = appreleasearr[i].preReleaseAuth; myArray[l]["3_PreReleaserName"] = appreleasearr[i].preReleaseName; myArray[l]["3_PreReleaseDate"] = r_preReleaseDate; myArray[l]["3_PreReleaseStatus"] = appreleasearr[i].preReleaseStatusCode; myArray[l]["3_SanctionReleaser"] = appreleasearr[i].postReleaseAuth; myArray[l]["3_SanctionReleaserName"] = appreleasearr[i].postReleaseName; myArray[l]["3_SanctionReleaseDate"] = r_postReleaseDate; myArray[l]["3_SanctionReleaseStatus"] = r_postReleaseStatusCode; };
                    };
                };
                $scope.sanObj = myArray; $scope.sanObj = $filter('orderBy')($scope.sanObj, 'TPADate'); $scope.exportObj = $scope.sanObj; $scope.$digest();
