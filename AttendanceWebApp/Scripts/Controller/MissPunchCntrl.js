...
app.controller("MissPunchController", function ($scope, $http, $filter) {
    ...
    $scope.ReleaseMissPunch = function (releaseStatusCode, appId, data) {
        ...
        xhr2.open('POST', $scope._Conpath +
            'MissedPunch/AppRelease?empUnqId=' + $('#myEmpUnqId').val() +
        ...
        xhr4.onreadystatechange = function () {
            if (xhr4.readyState === 4) {
                ...
                for (var i = 0; i < $scope.mpaData.length; i++) {
                    ...
                    $scope.mpaData[i]["empName"] = ($scope.mpaData[i] && $scope.mpaData[i].employee && $scope.mpaData[i].employee.empName) ? $scope.mpaData[i].employee.empName : "";
                ...
                for (var j = 0; j < arr.length; j++) {
                    ...
                    if (pType == "A") { myArray[cnt]["PunchType"] = "Attendance"; }
                    else { myArray[cnt]["PunchType"] = "In/Out"; }
                    cnt++;
                };
    ...
    /*-----START VALIDATION FOR ATTENDANCE PUNCH CORRECTIONS-----*/
    var loc = $("#myLoc").val();
    //Initial Load for Attendance Punch Correction
    ...
    //Validation for Miss Punch Application
    ...
    $scope.GetTripodPunch = function (gt) {
        ...
        if (diff >= 4) {
            document.getElementById("MessageBox").innerHTML =
                "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                "<strong>Limit for applying for missed punch is 3 days.</strong></div>";
            $('#MessageBox').show();
            document.getElementById("time").value = "";
            return false;
        };
        ...
        if (gt.empUnqId === null || (typeof (gt.empUnqId) === "undefined")) {
            GTP.open('GET', $scope._Conpath + 'Employee/gettripodpunch?empUnqId=' + $("#myEmpUnqId").val() + '&tDate=' +
                gt.time + '&ioFlag=' + gt.IO, true);
        }
        else {
            GTP.open('GET', $scope._Conpath + 'Employee/gettripodpunch?empUnqId=' + gt.empUnqId + '&tDate=' +
                gt.time + '&ioFlag=' + gt.IO, true);
        }
    ...
    //Get Performance Attendance Report
    ...
    /*-----END VALIDATION FOR ATTENDANCE PUNCH CORRECTIONS-----*/
    ...
    $scope.CreateAttdCorrEntry = function (data) {
        ...
        if ((loc === "IPU" || loc === "NKP") && (typeof (puncht) === "undefined")) {
            document.getElementById("MessageBox").innerHTML =
                "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
        ...
        else if ((loc !== "IPU" && loc !== "NKP") && (typeof (data.punchTime) === "undefined")) {
            document.getElementById("MessageBox").innerHTML =
                "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                "<strong>Please Select Punch Date Time</strong></div>";
            $('#MessageBox').show();
        ...
        if (loc === "IPU" || loc === "NKP") { jsonObj.AttdPunchTime = puncht; } else { jsonObj.AttdPunchTime = data.punchTime; };
            ...
            if (ace.readyState === 4 && ace.status === 200) {
                if (data.Reason === "Punched but not accepted by M/C" || data.Reason === "Error in the Punching Card") {
                    document.getElementById("InfoMessageBox").innerHTML =
                        "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                        "<h2>Please release from your leave releaser.</h2></div>";
                    $('#InfoMessageBox').show();
                };
                $("#cmbIOflag option:first").attr("selected", true);
    ...
    /*-----START CONTRATUAL EMPLOYEES ATTENDANCE CORRECTION-----*/
    ...
    $scope.GetSCEmpList = function () {
        ...
        sce.onreadystatechange = function () {
            if (sce.readyState === 4) {
                ...
                $scope.empdata = json;
                $scope.scempdata = Array.isArray($scope.empdata)
                    ? $scope.empdata.filter(function (item) {
                        return item.wrkGrp === "CONT";
                    })
                    : [];
                $scope.$digest();
            };
    ...
    $scope.CreateAttdCorrOther = function (data) {
        ...
        if ((typeof (data) === "undefined") || (typeof (data.Reason) === "undefined") || (typeof (data.IO) === "undefined")) {
            document.getElementById("MessageBox").innerHTML =
                ...
                "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                "<strong>Please Fill All Details.</strong></div>";
            $('#MessageBox').show(); document.getElementById("btnSubmit").disabled = false; return false;
        };
        var puncht = $scope.pTime;
        ...
        jsonObj.Reason = data.Reason;
        jsonObj.Remarks = "";
        jsonObj.ReleaseStrategy = data.empUnqId;
        jsonObj.ReleaseStatusCode = "N";
        jsonObj.InTime = null;
        jsonObj.InTimeUser = null;
        jsonObj.OutTime = null;
        ...
        ace.onreadystatechange = function () {
            if (ace.readyState === 4 && ace.status === 200) {
                ...
                if (data.Reason === "Punched but not accepted by M/C" ||
                    data.Reason === "Error in the Punching Card") {
                    document.getElementById("InfoMessageBox").innerHTML =
                        "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                        "<h2>Please release from your shift releaser.</h2></div>";
                    $('#InfoMessageBox').show();
                };
    ...
    /*-----END OTHERS ATTENDANCE CORRECTION-----*/
