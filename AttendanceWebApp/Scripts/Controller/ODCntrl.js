...
app.controller("ODController", function ($scope, $http) {
    ...
    }), $scope.currentPage = 1, $scope.itemsPerPage = 10, $scope.ResetView = function () {
        window.location.reload(!0);
    }, $scope.GetRelesaseStratey = function () {
        var rel = new XMLHttpRequest();
        rel.open("GET", $scope._Conpath + "ReleaseStrategy/GetReleaseStrategy?releaseGroup=" + $("#releaseGroupCode").val() + "&empUnqId=" + $("#myEmpUnqId").val(), !0),
        ...
        emp.open("GET", $scope._Conpath + "Employee/GetEmployee?empunqid=" + e_Code, !0),
            emp.setRequestHeader("Accept", "application/json"), emp.onreadystatechange = function () {
                if (4 === emp.readyState) {
                    ...
                    var date = new Date(), timeOptions = {
                        hour: "2-digit",
                        minute: "2-digit"
                    }, currentTime = date.toLocaleTimeString("it-IT", timeOptions);
                    document.getElementById("FromTm").value = currentTime, document.getElementById("ToTm").value = currentTime;
                }
            }, emp.send();
    },
