...
app.controller('ShortLeaveController',
    function($scope, $http, $filter) {
                ...
                if (xhr.readyState === 4) {
                    ...
                    for (var i = 0; i < mainarr.length; i++) {
                        for (var j = 0; j < mainarr[i]["leaveApplicationDetails"].length; j++) {
                            ...
                            var fromdate = mainarr[i]["leaveApplicationDetails"][j].fromDt;
                            var ftmp = (fromdate).slice(0, 10);
                            var f = ftmp.split("-");
                            var fdate = new Date(f[0], f[1] - 1, f[2]);
                            var predate = $('#previousMonthdate').val();
                            var pretemp = (predate).slice(1, 11);
                            var p = pretemp.split("-");
                            var pdate = new Date(p[0], p[1] - 1, p[2]);
                            pdate = new Date(pdate.getFullYear(), pdate.getMonth() - 1, 25);
                            if (ispostedflg === 'F' && fdate <= pdate) {
                                mainarr[i].openmonthflg = false;
                            } else {
                                mainarr[i].openmonthflg = true;
        ...
        $scope.PopulateData = function(LeaveAppId) {
            ...
            $scope.GetLeaveApplicationDetails();
        };
        $scope.GetLeaveApplicationDetails = function() {
            var xhr1 = new XMLHttpRequest();
            xhr1.open('GET',
                $scope._Conpath + 'LeaveApplication/GetLeaveApplication?leaveAppId=' + $scope.LeaveAppId,
                true);
            ...
            xhr1.onreadystatechange = function() {
                if (xhr1.readyState === 4) {
                    ...
                    if ($scope.postflg === false || typeof ($scope.postflg) === "undefined") {
                        $('#ConformModel').modal('show');
                    };
                    if ($scope.postflg === true) {
                        return false;
                    };
                };
            };
        ...
        $scope.LeaveCancel = function(entity) {
            ...
            var efdate = Date.parse(FromDate);
            var etdate = Date.parse(ToDate);
            var ltype;
            var chkfdt = "";
            var chktdt = "";
        ...
        $scope.FullLeaveCancel = function(lgrp, lappid) {
            ...
            if (retVal == true) {
                ...
                flc.open('POST',
                    $scope._Conpath + 'ShortLeave/CancelLeave?releaseGroupCode=' + lgrp + '&leaveAppId=' + lappid,
                    true);
                ...
                flc.onreadystatechange = function() {
                    if (flc.readyState === 4) {
                        ...
                        var sd1 = maildata["releaseGroupCode"];
                        var sd2 = maildata["leaveAppId"];
