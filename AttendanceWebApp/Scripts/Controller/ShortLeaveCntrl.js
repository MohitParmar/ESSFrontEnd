...
app.controller('ShortLeaveController', function ($scope, $http, $filter) {
            ...
            if (xhr.readyState === 4) {
                ...
                for (var i = 0; i < mainarr.length; i++) {
                    for (var j = 0; j < mainarr[i]["leaveApplicationDetails"].length; j++) {
                        ...
                        var ftmp = (fromdate).slice(0, 10); var f = ftmp.split("-"); var fdate = new Date(f[0], f[1] - 1, f[2]);

                        var predate = $('#previousMonthdate').val();
                        var pretemp = (predate).slice(1, 11); var p = pretemp.split("-"); var pdate = new Date(p[0], p[1] - 1, p[2]);
    ...
    $scope.LeaveCancel = function (entity) {
        ...
        var ToDate = entity.ToDt; var tarray = ToDate.split("-");
        ...
        if (Totaldays === null || Totaldays === '' || Totaldays === 0) { Totaldays = diff; }
        ...
        if (retVal == true) {
            ...
            flc.onreadystatechange = function () {
                if (flc.readyState === 4) {
                    ...
                    document.getElementById("MessageBox").innerHTML = "<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Leave Canceled Successfully.. </strong></div>";
                    $('#MessageBox').show();
                }
                else {
                    document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Leave Not Canceled</strong></div>";
                    $('#MessageBox').show();
                }
                $scope.GetShortLeaveLists();
            }
