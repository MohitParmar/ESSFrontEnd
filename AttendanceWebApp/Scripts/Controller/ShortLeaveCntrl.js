...
app.controller('ShortLeaveController', function ($scope, $http, $filter) {
    ...
    $scope.ToValidate = function () {
        ...
        if (diff > 1) { $("#HalfFlag").attr("disabled", true); } else { $("#HalfFlag").removeAttr("disabled"); }
    ...
    $scope.GetOpenMonth = function () { var mon = new XMLHttpRequest(); mon.open('GET', $scope._Conpath + 'OpenMonth/GetOpenMonth', true); mon.setRequestHeader("Content-type", "application/json"); mon.onreadystatechange = function () { if (mon.readyState === 4 && mon.status === 200) { var msg = mon.responseText; $('#previousMonthdate').val(msg); } }; mon.send(); };
    //Get Leave Application List By ECode
    $scope.GetShortLeaveLists = function () {
        $scope.GetOpenMonth();
        var xhr = new XMLHttpRequest(); xhr.open('GET', $scope._Conpath + 'ShortLeave/GetShortLeaves?empUnqId=' + $('#myEmpUnqId').val(), true);
            ...
            if (xhr.readyState === 4) {
                ...
                for (var i = 0; i < mainarr.length; i++) {
                    for (var j = 0; j < mainarr[i]["leaveApplicationDetails"].length; j++) {
                        var ispostedflg = mainarr[i]["leaveApplicationDetails"][j].isPosted;
                        var fromdate = mainarr[i]["leaveApplicationDetails"][j].fromDt; var ftmp = (fromdate).slice(0, 10); var f = ftmp.split("-"); var fdate = new Date(f[0], f[1] - 1, f[2]);
                        var predate = $('#previousMonthdate').val(); var pretemp = (predate).slice(1, 11); var p = pretemp.split("-");
                        var pdate = new Date(p[0], p[1] - 1, p[2]); pdate = new Date(pdate.getFullYear(), pdate.getMonth() - 1, 25);
                        //Add OpenMonth Flag Column
                        if (ispostedflg === 'F' && fdate <= pdate) { mainarr[i].openmonthflg = false; } else { mainarr[i].openmonthflg = true; }
                    }
    ...
    //Get Login User Leave Application Details by Application Id 
    $scope.GetLeaveApplicationDetails = function () {
        ...
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4) {
                ...
                for (var i = 0; i < leavedetail.length; i++) { var partialpost = leavedetail[i].isPosted; if (partialpost === 'P') { $scope.postflg = true; } else { $scope.postflg = false; } }
                if ($scope.postflg === false) { $scope.lappdata = jsonvar; $scope.$digest(); } else { alert("This Application is partially posted. Please Contact to HR Department."); }
    ...
    $scope.LeaveCancel = function (entity) {
        ...
        var tbldta = GetLeaveDetails()
        function GetLeaveDetails() { var tbldta = new Array(); $('#aliastable tr').each(function (row, tr) { tbldta[row] = { "leaveTypeCode": $(tr).find('td:eq(1)').text(), "fromDt": $(tr).find('td:eq(2)').text(), "toDt": $(tr).find('td:eq(3)').text() } }); return tbldta; };
        ...
        for (var i = 0; i < tbldta.length; i++) {
            var fdt = tbldta[i]["fromDt"]; var tdt = tbldta[i]["toDt"];
            var fdtarray = fdt.split("/"); fdt = fdtarray[2] + '/' + fdtarray[1] + '/' + fdtarray[0];
            var tdtarray = tdt.split("/"); tdt = tdtarray[2] + '/' + tdtarray[1] + '/' + tdtarray[0];
            fdt = Date.parse(fdt); tdt = Date.parse(tdt);
            if (efdate <= tdt && etdate >= fdt) { ltype = tbldta[i]["leaveTypeCode"]; if (i === 0) { chkfdt = fdt; } if (lnt === 1) { chktdt = tdt; } else { chktdt = tdt; } }
        }
        ...
        function storeTblValues() {
            ...
            TableData[0] = {
                "yearMonth": yearmonth, "leaveAppId": 0, "compCode": $('#myCompCode').val(), "wrkGrp": $('#myWrkGrp').val(), "leaveAppItem": 1, "leaveTypeCode": ltype
                , "fromDt": FromDate, "toDt": ToDate, "totalDays": Totaldays, "halfdayflag": false, "remarks": Remarks
            };
            ...
            jsonObj.statCode = $('#myStatCode').val(); jsonObj.secCode = $('#mySecCode').val(); jsonObj.catCode = $('#myCatCode').val();
            jsonObj.isHOD = $('#myIsHod').val(); jsonObj.releaseGroupCode = $('#releaseGroupCode').val(); jsonObj.releaseStrategy = "";
            jsonObj.releaseStatusCode = ""; jsonObj.addDt = d; jsonObj.addUser = $('#myEmpUnqId').val(); jsonObj.updDt = d; jsonObj.updUser = null; jsonObj.remarks = null;
            jsonObj.parentId = $scope.LeaveAppId; jsonObj.leaveApplicationDetails = TableData;
            return jsonObj;
        }
        ...
        xhr.onreadystatechange = function () {
            if (xhr.status === 201) {
                ...
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-success alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Cancellation Leave Created Sucesfully.. </strong></div>"; $('#MessageBox').show();
                ...
                jQuery('#btnClose').click();
                document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Leave Cancellation Not Created.. </strong></div>"; $('#MessageBox').show();
    ...
    $scope.FullLeaveCancel = function (lgrp, lappid) {
        ...
        if (retVal == true) {
            var flc = new XMLHttpRequest(); flc.open('POST', $scope._Conpath + 'ShortLeave/CancelLeave?releaseGroupCode=' + lgrp + '&leaveAppId=' + lappid, true);
            flc.setRequestHeader("Content-type", "application/json");
            flc.onreadystatechange = function () {
                if (flc.readyState === 4) {
                    //Auto Mail Sending
                    var json = JSON.parse(flc.responseText); var maildata = []; maildata = json; var relsdata = []; relsdata = maildata["applReleaseStatus"];
                    var relsauth = relsdata[0]["releaseAuth"]; var sd1 = maildata["releaseGroupCode"]; var sd2 = maildata["leaveAppId"];
                    var rlsmail = new XMLHttpRequest();
                    rlsmail.open('GET', $scope._Conpath + 'AutoMail/SendMail?releaseGroupCode=' + sd1 + '&id=' + sd2 + '&releaseAuth=' + relsauth, true);
