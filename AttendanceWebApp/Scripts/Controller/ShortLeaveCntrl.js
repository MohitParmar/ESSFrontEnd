...
app.controller('ShortLeaveController', function ($scope, $http, $filter) {
    ...
    $scope.GetOpenMonth = function () {
        ...
        mon.onreadystatechange = function () { if (mon.readyState === 4 && mon.status === 200) { var msg = mon.responseText; $('#previousMonthdate').val(msg); } };
        mon.send();
    };
    ...
    $scope.GetShortLeaveLists = function () {
        ...
        xhr.open('GET', $scope._Conpath + 'ShortLeave/GetShortLeaves?empUnqId=' + $('#myEmpUnqId').val(), true);
        xhr.setRequestHeader('Accept', 'application/json');
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                var json = JSON.parse(xhr.responseText);
                var mainarr = new Array(); mainarr = json;
                for (var i = 0; i < mainarr.length; i++) {
                    for (var j = 0; j < mainarr[i]["leaveApplicationDetails"].length; j++) {
                        ...
                        var predate = $('#previousMonthdate').val(); var pretemp = (predate).slice(1, 11); var p = pretemp.split("-"); var pdate = new Date(p[0], p[1] - 1, p[2]);
                        pdate = new Date(pdate.getFullYear(), pdate.getMonth() - 1, 25);
                        //Add OpenMonth Flag Column
                        if (ispostedflg === 'F' && fdate <= pdate) { mainarr[i].openmonthflg = false; } else { mainarr[i].openmonthflg = true; }
                    }
                }
                $scope.ldata = mainarr; $scope.ldata = $filter('orderBy')($scope.ldata, '-leaveAppId');
                $scope.curPage = 0; $scope.pageSize = 10; $scope.$digest();
            }
        };
        xhr.send();
    };
    ...
    $scope.GetLeaveApplicationDetails = function () {
        ...
        xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4) {
                ...
                if ($scope.postflg === false) { $scope.lappdata = jsonvar; $scope.$digest(); }
                else { alert("This Application is partially posted. Please Contact to HR Department."); }
    ...
    $scope.LeaveCancel = function (entity) {
        ...
        var yearmonth = year + (month.toString()); var Remarks = entity.Remarks; var FromDate = entity.FromDt;
        var farray = FromDate.split("-"); FromDate = farray[0] + '/' + farray[1] + '/' + farray[2];
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', $scope._Conpath + 'ShortLeave/CreateShortLeave', true);
    ...
    $scope.FullLeaveCancel = function (lgrp, lappid) {
        ...
        if (retVal == true) {
            ...
            flc.open('POST', $scope._Conpath + 'ShortLeave/CancelLeave?releaseGroupCode=' + lgrp + '&leaveAppId=' + lappid, true);
            ...
            flc.onreadystatechange = function () {
                if (flc.readyState === 4) {
                    ...
                    var relsauth = relsdata[0]["releaseAuth"]; var rlsmail = new XMLHttpRequest();
                    var sd1 = maildata["releaseGroupCode"]; var sd2 = maildata["leaveAppId"];
