...
app.controller('ShortLeaveController', function ($scope, $http, $filter) {
    ...
    jQuery.support.cors = true; $scope.ResetView = function () { window.location.reload(true); }; $scope.postflg;
    ...
    $scope.FullLeaveCancel = function (lgrp, lappid) {
        ...
        if (retVal == true) {
            ...
            flc.onreadystatechange = function () {
                ...
                } else { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>Leave Not Canceled</strong></div>"; $('#MessageBox').show(); }; $scope.GetShortLeaveLists();
            }; flc.send(); return true;
        } else { return false; };
    };
