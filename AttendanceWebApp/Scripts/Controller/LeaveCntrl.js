angular.module('myApp.Controllers').controller('LeaveController', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.SetLTListValue = function (value) {
        ...
        if (value === "EL" || value === "PL") {
            ...
            ser.open("GET",
                $scope._AttdConPath +
                "leavepost/leavecount?EmpUnqID=" + $("#myEmpUnqId").val() +
                "&LeaveType=" + value +
                "&Location=" + loc +
                "&tYear=" + tyear, !0);
            ...
            ser.onreadystatechange = function () {
                if (4 === ser.readyState) {
                    var json = JSON.parse(ser.responseText);
                    $scope.serData = json;
    ...
    $scope.ToValidate = function () {
        ...
        if (FromDate === "") {
            document.getElementById("MessageBox").innerHTML =
                ...
                "<strong>Please Select From Date First...</strong></div>";
            $('#MessageBox').show();
        ...
        if (diff === "NaN" || diff <= 0) {
            document.getElementById("MessageBox").innerHTML =
                "<div class='alert alert-danger alert-dismissable'>" +
                "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                "<strong>Please Select Proper Dates for Leave apply.</strong></div>";
            $('#MessageBox').show();
            document.getElementById("FromDt").value = "";
            document.getElementById("ToDt").value = "";
            document.getElementById("TotalDays").value = "";
    ...
    $scope.LeaveRequestData = function (entity) {
        ...
        if ((typeof (entity) === "undefined") ||
            ...
            alert("Please Fill All Required Details Step by Step...");
            return false;
        }
        var LeaveTypeCode = entity.LeaveTypeCode;
        var HalfDayFlag = false;
        var Totaldays = 0;
        var Remarks = entity.Remarks.replace("'", "").toUpperCase();
        ...
        if (LeaveTypeCode === "PL" && Totaldays < 3) {
            ...
            $("#LeaveType option:first").attr("selected", true);
            chktabldta = false;
            alert("PL cannot be less than 3 days");
            return;
        };
        ///Get Grid Data
        var d = new Date();
        ...
        function storeTblValues() {
            ...
            if (TableData.length > 1) {
                var prevohsl = '';
                var curohsl = '';
                prevohsl = TableData[c - 2]["leaveTypeCode"];
                curohsl = TableData[c - 1]["leaveTypeCode"];
                if ((curohsl === "OH" && prevohsl === "SL") ||
                    (curohsl === "SL" && prevohsl === "OH")) {
                    ...
                    document.getElementById("MessageBox").innerHTML =
                        "<div class='alert alert-warning alert-dismissable'>" +
                        "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                        "<strong>OH & SL can not be clubbed as per HR Policy..</strong></div>";
                    $('#MessageBox').show();
                    return;
                };
            }; var ltyp = '';
        ...
        if (chktabldta === true) {
            document.getElementById("MessageBox").innerHTML =
                "<div class='alert alert-warning alert-dismissable'>" +
                "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>" +
                "<strong>" + LeaveTypeCode + " is already exist please cannnot select same leave type</strong></div>";
            $('#MessageBox').show();
            document.getElementById("FromDt").value = "";
            document.getElementById("ToDt").value = "";
            document.getElementById("TotalDays").value = "";
