angular.module('myApp.Controllers').controller('LeaveController', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope._AttdConPath = ""; $scope._Conpath = ''; var loc = $("#myLoc").val();
    ...
    $scope.SetLTListValue = function (value) {
        ...
        if ((loc === "NSK" || loc === "NAG") && plCheck == "True" && (value === "EL" || value === "PL")) {
        ...
        if (value === "EL" || value === "PL") {
            var d = new Date(); var tyear = d.getFullYear();
            var ser = new XMLHttpRequest;
            ser.open("GET",
                $scope._AttdConPath + "leavepost/leavecount?EmpUnqID=" + $("#myEmpUnqId").val() +
                "&LeaveType=" + value + "&Location=" + loc + "&tYear=" + tyear, !0);
            ...
            ser.onreadystatechange = function () {
                if (4 === ser.readyState) {
                    ...
                    var cnt = parseInt($scope.serData) || 0;
                    if (cnt >= 4) {
                        alert("EL can be availed maximum of 4 times in a year.");
                        $scope.ResetView();
                    }; $scope.$digest();
                };
            }; ser.send();
    ...
    $scope.ToValidate = function () {
        ...
        if (FromDate === "") { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Please Select From Date First...</strong></div>"; $('#MessageBox').show(); document.getElementById("FromDt").value = ""; document.getElementById("ToDt").value = ""; document.getElementById("TotalDays").value = ""; document.getElementById("Remarks").value = ""; $('#HalfFlag').prop('checked', false); $("#LeaveType option:first").attr("selected", true); return false; };
        ...
        if (diff === "NaN" || diff <= 0) { document.getElementById("MessageBox").innerHTML = "<div class='alert alert-danger alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Please Select Proper Dates for Leave apply.</strong></div>"; $('#MessageBox').show(); document.getElementById("FromDt").value = ""; document.getElementById("ToDt").value = ""; document.getElementById("TotalDays").value = ""; document.getElementById("Remarks").value = ""; $('#HalfFlag').prop('checked', false); $("#LeaveType option:first").attr("selected", true); return false; };
    ...
    $scope.YearValidate = function () {
        ...
        var FromDate = chkFrom.value; var date1 = new Date(FromDate); var fyear = date1.getFullYear();
        var d = new Date(); var tyear = d.getFullYear();
    ...
    $scope.LeaveRequestData = function (entity) {
        ...
        var Remarks = entity.Remarks.replace("'", "").toUpperCase(); var FromDate = entity.FromDt; var chkTo = document.getElementById('ToDt'); var ToDate = chkTo.value;
        ...
        //Get Data From the Leave Application Form & store into Array
        function storeTblValues() {
            ...
            var row = $("<tr>" +
                ...
                "<td><input type='hidden' name='AliasLastNames' value='" + Remarks + "'>" + Remarks + "</td>" + "</tr>");
            ...
            if (TableData.length > 1) {
                ...
                if ((curohsl === "OH" && prevohsl === "SL") || (curohsl === "SL" && prevohsl === "OH")) {
                    var tables = document.getElementById('aliasTable'); var rowCounts = tables.rows.length; tables.deleteRow(rowCounts - 1); c = c - 1; chkohsl = true; document.getElementById("MessageBox").innerHTML = "<div class='alert alert-warning alert-dismissable'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong>OH & SL can not be clubbed as per HR Policy..</strong></div>"; $('#MessageBox').show(); return;
                };
            }; var ltyp = '';
        ...
        jQuery.support.cors = true; var xhr1 = new XMLHttpRequest(); xhr1.open('POST', $scope._Conpath + 'LeaveValidate/IsValid', true);
        xhr1.setRequestHeader("Content-type", "application/json"); xhr1.onreadystatechange = function () {
            if (xhr1.readyState === 4 && xhr1.status === 200) {
                ...
                document.getElementById("MessageBox").innerHTML =
                    ...
                    "</strong></div>"; $('#MessageBox').show(); document.getElementById("FromDt").value = ""; document.getElementById("ToDt").value = ""; document.getElementById("TotalDays").value = ""; document.getElementById("Remarks").value = ""; $('#HalfFlag').prop('checked', false); $("#LeaveType option:first").attr("selected", true);
            };
        }; xhr1.send(TableData);
    };
    ...
    $scope.createLeave = function () {
        ...
        function storeTblValues() {
            var TableData = new Array(); $('#aliasTable tr').each(function (row, tr) {
                TableData[row] = {
                    "yearMonth": yearmonth, "leaveAppId": 0, "compCode": $('#myCompCode').val(), "wrkGrp": $('#myWrkGrp').val(),
                    "leaveAppItem": $(tr).find('td:eq(0)').text(), "leaveTypeCode": $(tr).find('td:eq(1)').text(), "fromDt": $(tr).find('td:eq(2)').text(), "toDt": $(tr).find('td:eq(3)').text(), "totalDays": $(tr).find('td:eq(4)').text(), "halfdayflag": $(tr).find('td:eq(5)').text(), "remarks": $(tr).find('td:eq(6)').text()
                }
            }); TableData.shift();
            var larmks = "";
            if (TableData.length > 0) {
                for (var tdata = 0; tdata < TableData.length; tdata++) { var ltyp = TableData[tdata]["leaveTypeCode"]; if (ltyp === '') { return false; }; };
                for (var i = 0; i < TableData.length; i++) { larmks += TableData[i]["leaveTypeCode"] + "_"; };
            } else { return false; };
            jsonObj.yearMonth = yearmonth; jsonObj.leaveAppId = 0; jsonObj.empUnqId = $('#myEmpUnqId').val(); jsonObj.compCode = $('#myCompCode').val(); jsonObj.wrkGrp = $('#myWrkGrp').val(); jsonObj.unitCode = $('#myUnitCode').val(); jsonObj.deptCode = $('#myDeptCode').val(); jsonObj.statCode = $('#myStatCode').val(); jsonObj.catCode = $('#myCatCode').val(); jsonObj.isHOD = $('#myIsHod').val(); jsonObj.releaseGroupCode = $('#releaseGroupCode').val(); jsonObj.releaseStrategy = ""; jsonObj.releaseStatusCode = ""; jsonObj.addDt = dt; jsonObj.addUser = $('#myEmpUnqId').val(); jsonObj.clientIp = $('#myIPAddress').val(); jsonObj.updDt = dt; jsonObj.updUser = null; jsonObj.remarks = larmks; jsonObj.parentId = 0; jsonObj.leaveApplicationDetails = TableData; return jsonObj;
        };
        jQuery.support.cors = true; var xhr = new XMLHttpRequest(); xhr.open('POST', $scope._Conpath + 'LeaveApplication/CreateLeaveApplication', true); xhr.setRequestHeader("Content-type", "application/json");
    ...
    $scope.PLValidate = function () {
        var PLV = new XMLHttpRequest();
        PLV.open("PUT", $scope._Conpath + "/LeaveApplication/TogglePlFlag?empUnqId=" + $('#myEmpUnqId').val(), true);
        PLV.setRequestHeader("Content-Type", "application/json");
        PLV.onreadystatechange = function () {
            if (PLV.readyState === 4 && PLV.status === 200) {
                ...
                document.getElementById("MessageBox").innerHTML =
                    ...
                    "<strong>not activated.</strong></div>";
                $('#MessageBox').show();
            };
        };
        PLV.send();
