...
app.controller('IncomeTaxController', function ($scope, $http, $filter) {
    ...
    $scope.GetBankDetails = function () {
        ...
        var emp = new XMLHttpRequest(); emp.open('GET', $scope._Conpath + 'Employee/GetEmployee?empunqid=' + $('#eCode').val(), true);
    ...
    $scope.GetTaxDeclaration = function () {
        ...
        if (ecode === 0) {
            xhr1.open('GET', $scope._Conpath + 'TaxDeclaration/GetTaxDeclaration?empUnqId=' + $('#myEmpUnqId').val() + '&yearMonth=' + $('#hidyearMonth').val() +
                '&actualFlag=' + $('#hidactualFlag').val(), true);
        }
        else {
            xhr1.open('GET', $scope._Conpath + 'TaxDeclaration/GetTaxDeclaration?empUnqId=' + ecode + '&yearMonth=' + $('#hidyearMonth').val() + '&actualFlag=' +
                $('#hidactualFlag').val(), true);
            ...
            if (xhr1.readyState === 4) {
                ...
                if ($scope.taxdata.length > 0) {
                    for (var d = 0; d < $scope.taxdata.length; d++) {
                        ...
                        rentDetails = $scope.taxdata[d].rentDetails; ppfDetails = $scope.taxdata[d].ppfDetails; bankDeposits = $scope.taxdata[d].bankDeposits;
                        insuranceDetails = $scope.taxdata[d].insuranceDetails; nscDetails = $scope.taxdata[d].nscDetails;
                        mutualFundDetails = $scope.taxdata[d].mutualFundDetails; ulipDetails = $scope.taxdata[d].ulipDetails;
                        ...
                        if (taxdeclarationactualflag) {
                            ...
                            $('#a_childfees1').val($scope.taxdata[d].tuitionFeeChild1); $('#a_childfees2').val($scope.taxdata[d].tuitionFeeChild2);
                            $('#a_pensionscheme').val($scope.taxdata[d].notifiedPensionScheme);
                            $('#a_others1').val($scope.taxdata[d].others1Amount); $('#a_others2').val($scope.taxdata[d].others2Amount);
                            $('#a_equityscheme').val($scope.taxdata[d].rajivGandhiEquity); $('#a_mediclaimpremiumforfamily').val($scope.taxdata[d].medicalPremiumSelf);
                            $('#a_mediclaimpremiumforparents').val($scope.taxdata[d].medicalPremiumParents); $('#a_mediclaimpremiumforhealth').val($scope.taxdata[d].medicalPreventiveHealthCheckup);
                            $('#a_eduloan').val($scope.taxdata[d].educationLoanInterest);
                            $('#a_disablity').val($scope.taxdata[d].physicalDisability); $('#a_severedisablity').val($scope.taxdata[d].severeDisability);
                            $('#a_NPS').val($scope.taxdata[d].nationalPensionScheme);
                            $('#a_disdependent').val($scope.taxdata[d].disableDependent); $('#a_medicalexpenditure').val($scope.taxdata[d].medicalExpenditure);
                            $('#a_amountofinterest').val($scope.taxdata[d].interestOnLoan); $('#a_amountofinterestasperit').val($scope.taxdata[d].interestPreConstruction);
                            $('#a_optionalinterest').val($scope.taxdata[d].otherInterest); $('#a_optionalotherincome').val($scope.taxdata[d].otherIncomeAmount);
                            $scope.calmediclaim();
                            ...
                            $('#landlord').val($scope.taxdata[d].landLordName); $('#rentalpan').val($scope.taxdata[d].landLordPan); $scope.calcRent();
                            ...
                            $('#txt_others1').val($scope.taxdata[d].others1Desc); $('#txt_others2').val($scope.taxdata[d].others2Desc);
                            ...
                            $('#p_childfees1').val($scope.taxdata[d].tuitionFeeChild1); $('#p_childfees2').val($scope.taxdata[d].tuitionFeeChild2);
                            $('#p_pensionscheme').val($scope.taxdata[d].notifiedPensionScheme);
                            $('#p_others1').val($scope.taxdata[d].others1Amount); $('#p_others2').val($scope.taxdata[d].others2Amount);
                            $('#p_equityscheme').val($scope.taxdata[d].rajivGandhiEquity); $('#p_mediclaimpremiumforfamily').val($scope.taxdata[d].medicalPremiumSelf);
                            $('#p_mediclaimpremiumforparents').val($scope.taxdata[d].medicalPremiumParents); $('#p_mediclaimpremiumforhealth').val($scope.taxdata[d].medicalPreventiveHealthCheckup);
                            $('#p_eduloan').val($scope.taxdata[d].educationLoanInterest);
                            $('#p_disablity').val($scope.taxdata[d].physicalDisability); $('#p_severedisablity').val($scope.taxdata[d].severeDisability);
                            $('#p_NPS').val($scope.taxdata[d].nationalPensionScheme);
                            $('#p_disdependent').val($scope.taxdata[d].disableDependent); $('#p_medicalexpenditure').val($scope.taxdata[d].medicalExpenditure);
                            $('#p_amountofinterest').val($scope.taxdata[d].interestOnLoan); $('#p_amountofinterestasperit').val($scope.taxdata[d].interestPreConstruction);
                            $('#p_optionalinterest').val($scope.taxdata[d].otherInterest); $('#p_optionalotherincome').val($scope.taxdata[d].otherIncomeAmount);
                            $scope.calmediclaim();
                            if (rentDetails.length > 0) {
                                $('#renttotal').val($scope.taxdata[d].totalRentPaid);
    ...
    $scope.UpdateDetailsFin = function () {
        ...
        function storeTblValues() {
            ...
            jsonObj.rentDetails = RentData; jsonObj.ppfDetails = PPFData; jsonObj.bankDeposits = FDData; jsonObj.insuranceDetails = PremiumData; jsonObj.nscDetails = NSCData; jsonObj.mutualFundDetails = MutualFund; jsonObj.ulipDetails = ULIPData; jsonObj.sukanyaDetails = SSData; return jsonObj;
        };
        var tax = new XMLHttpRequest(); tax.open('POST', $scope._Conpath + 'TaxDeclaration/CreateTaxDeclaration', true); tax.setRequestHeader('Content-type', 'application/json');
        tax.onreadystatechange = function () {
            if (tax.readyState === 4 && tax.status === 200) {
    ...
    $scope.GetITReport = function () {
        $('#loading').removeClass("deactivediv"); $('#loading').addClass("activediv"); var gac = new XMLHttpRequest();
        gac.open('GET', $scope._Conpath + 'TaxDeclaration/GetTaxDeclarationConfig', true); gac.setRequestHeader('Accept', 'application/json');
        gac.onreadystatechange = function () {
            if (gac.readyState === 4) {
                ...
                itd.setRequestHeader('Accept', 'application/json'); itd.onreadystatechange = function () {
